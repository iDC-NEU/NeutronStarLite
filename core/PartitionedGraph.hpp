...
struct GnnUnit : torch::nn::Module {
  ...
  torch::Tensor forward2(torch::Tensor x) { return torch::sigmoid(x); }
...
public:
  ...
  void readData_txt(Graph<Empty> *graph, std::string feature_fname,
  ...
  void readData_binary(Graph<Empty> *graph, std::string feature_fname,
                       std::string label_fname) {
    std::ifstream input_feature(feature_fname, std::ios::in);
    if (!input_feature.is_open()) {
      std::cout << "open " << feature_fname << " fail!" << std::endl;
...
public:
  ...
  void updateX(int layer_, torch::Tensor src) { x[layer_] = src; }

  template <typename T_l> void registLabel(T_l *label, int start, int rownum) {
...
public:
  ...
  GTensor(Graph<Empty> *graph, Embeddings<t_v, t_l> *embedding,
    ...
    graph_->process_vertices<t_v>( // init  the vertex state.
        [&](VertexId vtx) {
          graph->in_degree_for_backward[vtx] =
              graph->in_degree_for_backward[vtx] + 1; // local
          graph->out_degree_for_backward[vtx] =
  ...
  torch::Tensor applyVertexPre(torch::Tensor source,
  ...
  template <int CURRENT_LAYER_SIZE> torch::Tensor Propagate(int local_layer) {
  ...
  torch::Tensor Sync_data_new(torch::Tensor &data) {
    std::cout << "start" << start_ << " " << data.accessor<t_v, 2>().size(1)
              << std::endl;
    memset(data_coomm, 0, sizeof(t_v) * CURRENT_LAYER_SIZE * graph_->vertices);
    graph_->process_edges<
        int, nodeVector<CURRENT_LAYER_SIZE>>( // For EACH Vertex Processing
        ...
        active_);
    return torch::from_blob(data_coomm + (start_)*CURRENT_LAYER_SIZE,
                            {rownum_, CURRENT_LAYER_SIZE}, torch::kFloat);
  }
  ...
  void setGradFromTensor(torch::Tensor new_tensor) { pre_grad = new_tensor; }
  torch::Tensor v(int local_layer) { return value_local[local_layer]; }
  torch::Tensor require_grad() { return grad; }
};
