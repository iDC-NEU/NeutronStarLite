...
namespace nts {
    struct ntsEdgeTensor{
        ...
        void initEdgeTensorFromTensor(int feature_size,graph_Tensor_Segment_pinned* subgraph_,NtsScheduler* ntsscheduler_,NtsVar &e_tensor){
            ...
            for(VertexId vtx=subgraph->dst_range[0];vtx<subgraph->dst_range[1];vtx++){
                ...
                edgeTensorIndexedbyDst[offset]=ntsscheduler->NewLeafTensor(data_buffer+eid_start*size_1,{(eid_end-eid_start),size_1},torch::DeviceType::CPU);           
            }
            dim=2;
    ...
    struct ntsVertexTensor{
        ...
        void initVertexTensorFromTensor(int feature_size,graph_Tensor_Segment_pinned* subgraph_,NtsScheduler* ntsscheduler_,NtsVar &e_tensor){
            ...
            for(VertexId vtx=subgraph->dst_range[0];vtx<subgraph->dst_range[1];vtx++){
                int offset=vtx-subgraph->dst_range[0];
                vertexTensorIndexedbyDst[offset]=ntsscheduler->NewLeafTensor(data_buffer+offset*size_1,{1,size_1},torch::DeviceType::CPU);           
            }
            dim=2;
