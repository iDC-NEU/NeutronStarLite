...
template <int size_> struct nodeVector {
...
//} weightVector;
template <typename t_v, int SIZE_> struct EdgeFeature { t_v data[SIZE_]; };
...
template <typename T_v, typename T_l> class Embeddings {
public:
  Embeddings() {}
  T_v *start_v = NULL;
    ...
    for (int i = 0; i < graph->owned_vertices; i++) {
      ...
      for (int j = 0; j < SIZE_LAYER_1; j++) {
        if (con[i + bias].att[j] != 0.0) {
          ...
          cf->value_list[cf->column_index[i] + (local_index)] =
              con[i + bias].att[j];
          local_index++;
        }
      }
  ...
  void init(Graph<Empty> *graph) {
    start_v = new ValueType
        [graph->vertices *
         SIZE_LAYER_1]; // graph->alloc_vertex_array<float>(VECTOR_LENGTH);

    label = graph->alloc_vertex_array<T_l>();
    rownum = (graph->partition_offset[graph->partition_id + 1] -
  ...
  void readlabel1(Graph<Empty> *graph) {
    ...
    while (input >> con[numOfData].id) {
      ...
      //   std::cout<<"lable: "<<con[numOfData].label<<"
      //   "<<numOfData<<std::endl;
      numOfData++;
    }
    //    std::cout<<"finish1"<<std::endl;
  ...
  void readlabel_google(Graph<Empty> *graph) {
    std::string str;
    con = new content[NODE_NUMBER];
    std::ifstream input("/home/wangqg/NetBeansProjects/DataPreProcessing/"
                        "google/google_contents",
                        std::ios::in);
    if (!input.is_open()) {
      std::cout << "open file fail!" << std::endl;
      return;
...
public:
    ...
    std::vector<NtsVar> x; // after graph engine;
    std::vector<NtsVar> y;
    std::vector<NtsVar> localGrad;
    std::vector<NtsVar> backwardGrad;
    NtsVar target; //read label
    NtsVar loss;
    NtsVar in_degree;
    NtsVar out_degree;
    ...
    void updateX(int layer_, NtsVar src)
    {
        x[layer_] = src;
    }
...
*/
void init_parameter(Network<ValueType> *comm, Graph<Empty> *graph, GnnUnit *gnn,
                    Embeddings<ValueType, long> *embedding) {
...
NtsVar unified_parameter(Network<ValueType> *comm, NtsVar res) {
  // return res;
  int r = res.accessor<ValueType, 2>().size(0);
  int c = res.accessor<ValueType, 2>().size(1);
  comm->wrtBuffertoBuff(res.accessor<ValueType, 2>().data(), r,
                        c); // write para to send buffer
...
template <typename t_v, typename t_l, int MAX_EMBEDDING_SIZE> class GTensor {
...
public:
  ...
  t_v **value_buffer;
  t_v **grad_buffer;
  std::vector<NtsVar> value_local;
  std::vector<NtsVar> grad_local;
  ...
  inline void cpToCurr(int vtx, t_v *src) {
    memcpy(curr_local[vtx - start_].data, src,
           sizeof(nodeVector<MAX_EMBEDDING_SIZE>));
  }
  template <int EBDING_SIZE> inline void cpToCurr(int vtx, t_v *src) {
    memcpy(((nodeVector<EBDING_SIZE> *)curr_local)[vtx - start_].data, src,
           sizeof(nodeVector<EBDING_SIZE>));
  }
  template <int EBDING_SIZE> inline void cpToNext(int vtx, t_v *src) {
    memcpy(((nodeVector<EBDING_SIZE> *)next_local)[vtx - start_].data, src,
           sizeof(nodeVector<EBDING_SIZE>));
  }

  inline void cpToNext(int vtx, t_v *src) {
    memcpy(next_local[vtx - start_].data, src,
           sizeof(nodeVector<MAX_EMBEDDING_SIZE>));
  }
  ...
  template <int EBDING_SIZE> inline void addToNext(int vtx, t_v *src) {
  ...
  inline void addToNextValue(int layer, int vtx, t_v *src) {
    for (int i = 0; i < EBDING_SIZE; i++) {
      ((nodeVector<EBDING_SIZE> *)value_buffer[layer])[vtx - start_].data[i] +=
          src[i];
    }
  }
  template <int EBDING_SIZE>
  inline void addToNextGrad(int layer, int vtx, t_v *src) {
    for (int i = 0; i < EBDING_SIZE; i++) {
      ((nodeVector<EBDING_SIZE> *)grad_buffer[layer])[vtx - start_].data[i] +=
          src[i];
    }
  }

  inline void zeroValue(int layer) {
    memset(value_buffer[layer], 0,
           sizeof(t_v) * size_at_layer[layer] * rownum_);
  ...
  GTensor(Graph<Empty> *graph, Embeddings<t_v, t_l> *embedding,
          VertexSubset *active, int layer, std::vector<int> size_) {
    graph_ = graph;
    embedding_ = embedding;
    active_ = active;
    start_ = graph->partition_offset[graph->partition_id];
    rownum_ = graph->partition_offset[graph->partition_id + 1] -
              graph->partition_offset[graph->partition_id];
    layer_ = layer;
    current_layer = -1;
    ...
    curr_local =
        graph->alloc_vertex_array_local<nodeVector<MAX_EMBEDDING_SIZE>>();
    next_local =
        graph->alloc_vertex_array_local<nodeVector<MAX_EMBEDDING_SIZE>>();
    value_local.clear();
    grad_local.clear();
  ...
  NtsVar applyVertex(int local_layer, NtsVar vertex_data) {
    NtsVar destination;
    bool s = value_local[local_layer].accessor<ValueType, 2>().size(0) ==
             vertex_data.accessor<ValueType, 2>().size(0);
    bool t = value_local[local_layer].accessor<ValueType, 2>().size(1) ==
             vertex_data.accessor<ValueType, 2>().size(1);
  ...
  NtsVar applyVertexPre(NtsVar source, NtsVar vertex_data) {
    NtsVar destination;
    bool s = source.accessor<ValueType, 2>().size(0) ==
             vertex_data.accessor<ValueType, 2>().size(0);
    bool t = source.accessor<ValueType, 2>().size(1) ==
             vertex_data.accessor<ValueType, 2>().size(1);
    // assert(source.accessor<float,2>().size(0)==vertex_data.accessor<float,2>().size(0));
    // assert(source.accessor<float,2>().size(1)==vertex_data.accessor<float,2>().size(1));
    assert((s && t) == 1);
  ...
  template <int CURRENT_LAYER_SIZE> NtsVar Propagate(int local_layer) {
    zeroValue(local_layer);

    graph_->process_vertices<t_v>( // init  the vertex state.
        [&](VertexId vtx) {
          cpToCurr<CURRENT_LAYER_SIZE>(
              vtx, pre_value.accessor<t_v, 2>().data() +
                       (vtx - start_) * CURRENT_LAYER_SIZE); // local
          return (ValueType)1;
        },
        active_);
    // printf("unfinish propagate at layer [%d]\n",local_layer);
    graph_->process_edges<
        int, nodeVector<CURRENT_LAYER_SIZE>>( // For EACH Vertex Processing
        [&](VertexId dst, VertexAdjList<Empty> incoming_adj) { // pull
          ...
          for (AdjUnit<Empty> *ptr = incoming_adj.begin;
            ...
            for (int i = 0; i < CURRENT_LAYER_SIZE; i++) {
              ...
              sum.data[i] +=
                  ((nodeVector<CURRENT_LAYER_SIZE> *)curr_local)[src - start_]
                      .data[i] /
                  ((ValueType)std::sqrt(graph_->out_degree_for_backward[src]) *
    ...
    value_local[local_layer] =
        torch::from_blob(value_buffer[local_layer],
                         {rownum_, CURRENT_LAYER_SIZE}, torch::kFloat);
  ...
  NtsVar Propagate_inplace(int local_layer, ValueType *input) {
    zeroValue(local_layer);
    ...
    graph_->process_edges<
        ...
        [&](VertexId dst, VertexAdjList<Empty> incoming_adj) { // pull
          ...
          for (AdjUnit<Empty> *ptr = incoming_adj.begin;
            ...
            VertexId src = ptr->neighbour;
            for (int i = 0; i < CURRENT_LAYER_SIZE; i++) {
              if (0.0 != ((nodeVector<CURRENT_LAYER_SIZE> *)input)[src - start_]
                             .data[i])
                sum.data[i] +=
                    ((nodeVector<CURRENT_LAYER_SIZE> *)input)[src - start_]
                        .data[i] /
                    ((ValueType)std::sqrt(
                         graph_->out_degree_for_backward[src]) *
                     (ValueType)std::sqrt(graph_->in_degree_for_backward[dst]));
            }
          }
          graph_->emit(dst, sum);
  ...
  template <int CURRENT_LAYER_SIZE> void Propagate_backward(int local_layer) {
    zeroGrad(local_layer);               // local
    graph_->process_vertices<ValueType>( // init  the vertex state.
        [&](VertexId vtx) {
          cpToNext<CURRENT_LAYER_SIZE>(
              vtx, pre_grad.accessor<t_v, 2>().data() +
                       (vtx - start_) * CURRENT_LAYER_SIZE); // local
          return 1;
        },
        active_);
    // start graph engine.
    graph_->process_edges_backward<
        int, nodeVector<CURRENT_LAYER_SIZE>>( // For EACH Vertex Processing
        [&](VertexId src, VertexAdjList<Empty> outgoing_adj) { // pull
          nodeVector<CURRENT_LAYER_SIZE> sum;
          memset(sum.data, 0, sizeof(ValueType) * CURRENT_LAYER_SIZE);
          for (AdjUnit<Empty> *ptr = outgoing_adj.begin;
               ptr != outgoing_adj.end; ptr++) { // pull model
            VertexId dst = ptr->neighbour;
            for (int i = 0; i < CURRENT_LAYER_SIZE; i++) {
              // if(0!=((nodeVector<CURRENT_LAYER_SIZE>*)next_local)[dst-start_].data[i])
              sum.data[i] +=
                  ((nodeVector<CURRENT_LAYER_SIZE> *)next_local)[dst - start_]
                      .data[i] /
                  ((ValueType)std::sqrt(graph_->out_degree_for_backward[src]) *
                   (ValueType)std::sqrt(graph_->in_degree_for_backward[dst]));
            }
    ...
    grad_local[local_layer] = torch::from_blob(
        grad_buffer[local_layer], {rownum_, CURRENT_LAYER_SIZE}); // local
  ...
  template <int CURRENT_LAYER_SIZE>
  void Sync_data_optional(NtsVar &data, NtsVar &data_to) {
    data_to.zero_();

    printf("new sync\n");
    // std::cout<<"start"<<start_<<"
    // "<<data.accessor<t_v,2>().size(1)<<std::endl;
    // memset(data_coomm,0,sizeof(t_v)*CURRENT_LAYER_SIZE*graph_->vertices);
    graph_->process_edges<
        int, nodeVector<CURRENT_LAYER_SIZE>>( // For EACH Vertex Processing
        [&](VertexId src, VertexAdjList<Empty>
                              outgoing_adj) { // pull
                                              // if(src>=2200){
                                              //     std::cout<<"yes "<<src<<"
                                              //     is called"<<std::endl;
          ...
          if (graph_->out_degree_for_backward[src] <= 3) {
            memcpy(sum.data,
                   data.accessor<t_v, 2>().data() + (src)*CURRENT_LAYER_SIZE,
                   sizeof(t_v) * CURRENT_LAYER_SIZE);
            graph_->emit(src, sum);
          }
        },
        [&](VertexId src, nodeVector<CURRENT_LAYER_SIZE> msg) {
          ...
          for (int i = 0; i < CURRENT_LAYER_SIZE; i++) {
            ...
            data_to.accessor<t_v, 2>()
                .data()[(src - start_) * CURRENT_LAYER_SIZE + i] += msg.data[i];
          }
          return 0;
        },
        active_);
  ...
  template <int CURRENT_LAYER_SIZE>
  void Sync_data(NtsVar &data, NtsVar &data_to) {
    data_to.zero_();
    graph_->process_edges_simple<
        int, nodeVector<CURRENT_LAYER_SIZE>>( // For EACH Vertex Processing
        [&](VertexId src, VertexAdjList<Empty> outgoing_adj) { // pull
          nodeVector<CURRENT_LAYER_SIZE> sum;
          // if(graph_->out_degree_for_backward[src]<=3){
        ...
        active_);
    // return
    // torch::from_blob(data_coomm+(start_)*CURRENT_LAYER_SIZE,{rownum_,CURRENT_LAYER_SIZE},torch::kFloat);
  }

  template <int CURRENT_LAYER_SIZE>
  void Sync_data_gpu(NtsVar &data, NtsVar &data_to, bool selective = false) {
    if (!selective) { // original communication
      graph_->process_edges_with_GPU_aggregator<
          int, nodeVector<CURRENT_LAYER_SIZE>>( // For EACH Vertex Processing
          [&](VertexId src, VertexAdjList<Empty> outgoing_adj) { // pull
            nodeVector<CURRENT_LAYER_SIZE> sum;
            memcpy(sum.data,
          ...
          data_to.packed_accessor<float, 2>().data(), CURRENT_LAYER_SIZE,
          active_);
    } else { // selective comunication
      graph_->process_edges_with_GPU_aggregator<
          int, nodeVector<CURRENT_LAYER_SIZE>>( // For EACH Vertex Processing
          [&](VertexId src, VertexAdjList<Empty> outgoing_adj) { // pull
            nodeVector<CURRENT_LAYER_SIZE> sum;
  ...
  void Process_GPU_overlap_explict(
      NtsVar &X, NtsVar &Y,
      std::vector<CSC_segment_pinned *> &graph_partitions) {
    int current_layer_size =
        graph_->gnnctx->layer_size[graph_->rtminfo->curr_layer];
    ...
    if (!selective) { // original communication
      ...
      graph_->compute_sync_explict<int, float>(
          ...
          [&](VertexId src, VertexAdjList<Empty> outgoing_adj) { // pull
            // nodeVector<CURRENT_LAYER_SIZE> sum;
            // memcpy(sum.data, graph_->output_cpu_buffer +
            // (src)*current_layer_size, sizeof(t_v) * CURRENT_LAYER_SIZE);
            graph_->emit_buffer(
                src, graph_->output_cpu_buffer + (src)*current_layer_size,
                current_layer_size);
          },
  ...
  void Process_GPU_overlap_sync_compute_explict(
      NtsVar &X, NtsVar &Y,
      std::vector<CSC_segment_pinned *> &graph_partitions) {
    int current_layer_size =
        graph_->gnnctx->layer_size[graph_->rtminfo->curr_layer];
    ...
    if (!selective) { // original communication
      ...
      graph_->sync_compute<int, float>(
          ...
          [&](VertexId src) { // push
            ...
            graph_->emit_buffer(src,
                                X_buffered + (src - graph_->gnnctx->p_v_s) *
                                                 current_layer_size,
                                current_layer_size);
          },
          Y.packed_accessor<float, 2>().data());
      ...
      graph_->sync_compute<int, float>(
          ...
          [&](VertexId src) { // pull
            if (!graph_->RepVtx[layer]->get_bit(src)) {
              graph_->emit_buffer(src, X_buffered + (src)*current_layer_size,
                                  current_layer_size);
            }
          },
          Y.packed_accessor<float, 2>().data());
  ...
  void GraphPropagateForwardEdgeComputation(
      NtsVar &src_input_origin, NtsVar &dst_output,
      std::vector<CSC_segment_pinned *> &graph_partitions,
    ...
    graph_->sync_compute_edge_computation<int, float>(
        src_input_origin, graph_partitions,
        [&](VertexId src) {
          graph_->emit_buffer(src,
                              X_buffered + (src - graph_->gnnctx->p_v_s) *
                                               current_layer_size,
        ...
        [&](NtsVar &d_i) { return PreComputation(d_i); },
        [&](NtsVar &s_i, NtsVar &s_i_t, NtsVar &d_i, NtsVar &d_i_t,
            NtsScheduler *nts) {
  ...
  void GraphPropagateBackwardEdgeComputation(
  ...
  template <int CURRENT_LAYER_SIZE> NtsVar Test_Propagate(int local_layer) {
    zeroCurr(); // local
    zeroNext(); // local
    zeroValue(local_layer);
    ...
    graph_->process_vertices<ValueType>( // init the vertex state.
        [&](VertexId vtx) {
          memcpy(
              this->value_buffer[local_layer] +
                  CURRENT_LAYER_SIZE * (vtx - start_),
              ((nodeVector<CURRENT_LAYER_SIZE> *)next_local)[vtx - start_].data,
              sizeof(t_v) * CURRENT_LAYER_SIZE);
          return 0;
        },
        active_);
    ...
    if (graph_->partition_id == 0) {
      ...
      for (int i = 0; i < graph_->vertices; i++) {
        if (value_buffer[local_layer][i * CURRENT_LAYER_SIZE + 3] -
                (ValueType)graph_->in_degree_for_backward[i] <=
            0.00001)
  ...
  void setValueFromTensor(NtsVar new_tensor) {
  ...
  void setValueFromNative(t_v *data, int offset) {
    pre_value =
        torch::from_blob(data + offset, {rownum_, MAX_LAYER}, torch::kFloat);
    zeroCurr(); // local
  }
  void setValueFromNative(t_v *data, int offset, int length_) {
    pre_value =
        torch::from_blob(data + offset, {rownum_, length_}, torch::kFloat);
    zeroCurr(); // local
  }
  ...
  void setGradFromTensor(NtsVar new_tensor) { pre_grad = new_tensor; }
  NtsVar v(int local_layer) { return value_local[local_layer]; }
  NtsVar require_grad() { return grad; }
};

void generate_edge_list_Tensor(Graph<Empty> *graph,
                               std::vector<edge_list *> &graph_partitions,
                               int batch_size) {
  ...
  for (int i = 0; i < graph->graph_shard.size(); i++) {
    ...
    // if(graph->partition_id==1)std::cout<<"test_range in
    // generate"<<graph->graph_shard[i]->src_range[0]<<"
    // "<<graph->graph_shard[i]->src_range[1]<<std::endl;
    // if(graph->partition_id==1)std::cout<<"test_range in
    // generate"<<graph->graph_shard[i]->dst_range[0]<<"
    // "<<graph->graph_shard[i]->dst_range[1]<<std::endl;
    graph_partitions[i]->batch_size = batch_size;
    graph_partitions[i]->feature_size = SIZE_LAYER_1;
    graph_partitions[i]->dst =
        torch::from_blob(graph->graph_shard[i]->dst_delta,
                         {1, graph_partitions[i]->edge_size}, torch::kInt32);
    graph_partitions[i]->src =
        torch::from_blob(graph->graph_shard[i]->src_delta,
                         {1, graph_partitions[i]->edge_size}, torch::kInt32);
    graph_partitions[i]->weight_buffer =
        new float[graph_partitions[i]->edge_size];
    //   std::cout<<"generate_edge_list"<<graph_partitions[i]->edge_size<<"
    //   "<<std::endl;
    for (int j = 0; j < graph_partitions[i]->edge_size; j++) {
      ...
      graph_partitions[i]->weight_buffer[j] =
          (ValueType)std::sqrt(graph->out_degree_for_backward[v_src]) *
          (ValueType)std::sqrt(graph->in_degree_for_backward[v_dst]);
    }
    graph_partitions[i]->weight =
        torch::from_blob(graph_partitions[i]->weight_buffer,
                         {1, graph_partitions[i]->edge_size}, torch::kFloat);
  }
  // std::cout<<"generate_edge_list_Tensor"<<graph->graph_shard.size()<<std::endl;
  // std::cout<<"graph_edges "<<graph_partitions[0]->edge_size<<"
  // "<<graph->edges<<std::endl;
}
void generate_CSC_Segment_Tensor(Graph<Empty> *graph,
                                 std::vector<CSC_segment *> &graph_partitions,
                                 int batch_size, bool overlap = false) {
  graph_partitions.clear();
  ...
  for (int i = 0; i < graph->graph_shard.size(); i++) {
    ...
    graph_partitions[i]->src_range[0] = graph->graph_shard[i]->src_range[0];
    graph_partitions[i]->src_range[1] = graph->graph_shard[i]->src_range[1];
    int column_offset_size = graph->graph_shard[i]->dst_range[1] -
                             graph->graph_shard[i]->dst_range[0] + 1;
    graph_partitions[i]->batch_size = graph->graph_shard[i]->dst_range[1] -
                                      graph->graph_shard[i]->dst_range[0];
    ;
    graph_partitions[i]->feature_size = SIZE_LAYER_1;

    graph_partitions[i]->column_offset =
        torch::zeros({1, column_offset_size}, torch::kInt32);
    graph_partitions[i]->row_indices =
    ...
    for (int j = 0; j < column_offset_size - 1; j++) {
      ...
      graph_partitions[i]->column_offset.accessor<int, 2>().data()[j + 1] =
          tmp_column_offset[j + 1];
    }
    ...
    for (int j = 0; j < graph_partitions[i]->edge_size; j++) {
      ...
      VertexId v_dst = graph->graph_shard[i]->dst_delta[j] -
                       graph_partitions[i]->dst_range[0];
      graph_partitions[i]
          ->row_indices.accessor<int, 2>()
          .data()[tmp_column_offset[v_dst]] = v_src;
      graph_partitions[i]->weight_buffer[tmp_column_offset[v_dst]++] =
          1; //(ValueType)std::sqrt(graph->out_degree_for_backward[v_src]) *
             //(ValueType)std::sqrt(graph->in_degree_for_backward[v_dst]);
    }
    graph_partitions[i]->edge_weight =
        torch::from_blob(graph_partitions[i]->weight_buffer,
                         {1, graph_partitions[i]->edge_size}, torch::kFloat);
  }
...
void generate_CSC_Segment_Tensor_pinned(
    Graph<Empty> *graph, std::vector<CSC_segment_pinned *> &graph_partitions,
    bool overlap = false) {
  graph_partitions.clear();
  int *tmp_column_offset = new int[graph->vertices + 1];
  memset(tmp_column_offset, 0, sizeof(int) * (graph->vertices + 1));
  ...
  for (int i = 0; i < graph->graph_shard.size(); i++) {
    ...
    graph_partitions[i]->src_range[0] = graph->graph_shard[i]->src_range[0];
    graph_partitions[i]->src_range[1] = graph->graph_shard[i]->src_range[1];
    long column_offset_size = graph->graph_shard[i]->dst_range[1] -
                              graph->graph_shard[i]->dst_range[0] + 1;

    graph_partitions[i]->batch_size = graph->graph_shard[i]->dst_range[1] -
                                      graph->graph_shard[i]->dst_range[0];
    graph_partitions[i]->feature_size = graph->gnnctx->layer_size[0];
    graph_partitions[i]->column_offset = (VertexId *)cudaMallocPinned(
        column_offset_size *
        sizeof(
            VertexId)); // torch::zeros({1,column_offset_size},torch::kInt32);
    graph_partitions[i]->row_indices = (VertexId *)cudaMallocPinned(
        (graph_partitions[i]->edge_size + 1) *
        sizeof(
            VertexId)); // torch::zeros({1,graph_partitions[i]->edge_size},torch::kInt32);
    graph_partitions[i]->destination = (long *)cudaMallocPinned(
        (graph_partitions[i]->edge_size + 1) * sizeof(long));
    graph_partitions[i]->source = (long *)cudaMallocPinned(
        (graph_partitions[i]->edge_size + 1) * sizeof(long));
    graph_partitions[i]->edge_weight = (float *)cudaMallocPinned(
        (graph_partitions[i]->edge_size + 1) * sizeof(VertexId));

    graph_partitions[i]->column_offset_gpu =
        (VertexId *)getDevicePointer(graph_partitions[i]->column_offset);
    graph_partitions[i]->row_indices_gpu =
        (VertexId *)getDevicePointer(graph_partitions[i]->row_indices);
    graph_partitions[i]->edge_weight_gpu =
        (float *)getDevicePointer(graph_partitions[i]->edge_weight);
    ...
    for (int j = 0; j < graph_partitions[i]->edge_size; j++) {
      ...
      graph_partitions[i]->destination[tmp_column_offset[v_dst]] =
          (long)(v_dst + graph_partitions[i]->dst_range[0]);
  ...
  if (overlap) {
    ...
    for (int i = 0; i < graph_partitions.size(); i++) {
      max_batch_size =
          std::max(max_batch_size, graph_partitions[i]->batch_size);
    }
    graph->output_gpu_buffered =
        torch::zeros({max_batch_size, graph->gnnctx->max_layer},
...
void generate_Forward_Segment_Tensor_pinned(
    Graph<Empty> *graph, std::vector<CSC_segment_pinned *> &graph_partitions,
    bool overlap = false) {
  graph_partitions.clear();
  int *tmp_column_offset = new int[graph->vertices + 1];
  ...
  for (int i = 0; i < graph->graph_shard.size(); i++) {
    ...
    long column_offset_size = graph_partitions[i]->dst_range[1] -
                              graph_partitions[i]->dst_range[0] + 1;

    long row_offset_size = graph_partitions[i]->src_range[1] -
    ...
    graph_partitions[i]->row_offset =
        (VertexId *)cudaMallocPinned(row_offset_size * sizeof(VertexId)); ///
    graph_partitions[i]->column_indices = (VertexId *)cudaMallocPinned(
        (graph_partitions[i]->edge_size + 1) * sizeof(VertexId)); ///
    graph_partitions[i]->edge_weight_backward = (float *)cudaMallocPinned(
        (graph_partitions[i]->edge_size + 1) * sizeof(VertexId)); ///
    ...
    graph_partitions[i]->row_offset_gpu =
        (VertexId *)getDevicePointer(graph_partitions[i]->row_offset); ///
    graph_partitions[i]->column_indices_gpu =
        (VertexId *)getDevicePointer(graph_partitions[i]->column_indices); ///
    graph_partitions[i]->edge_weight_backward_gpu = (float *)getDevicePointer(
        graph_partitions[i]->edge_weight_backward); ///

    graph_partitions[i]->source_gpu =
        (long *)getDevicePointer(graph_partitions[i]->source); ///
    graph_partitions[i]->destination_gpu =
        (long *)getDevicePointer(graph_partitions[i]->destination); ///
...
void propagate_forward_gpu_shard(Graph<Empty> *graph, NtsVar input_cpu,
                                 NtsVar &output_cpu,
                                 std::vector<edge_list *> &graph_partitions,
                                 int feature_size) {
  NtsVar output_gpu;
  ...
  NtsVar weight_gpu;
  int src_blocks =
      graph->owned_vertices / graph_partitions[0]->batch_size +
      (int)((graph->owned_vertices % graph_partitions[0]->batch_size) > 0);
  int dst_blocks = graph_partitions.size() / src_blocks;
  std::cout << src_blocks << " " << dst_blocks << std::endl;
  output_cpu.zero_();
  output_gpu =
      torch::zeros({graph_partitions[0]->batch_size, feature_size},
                   at::TensorOptions().device_index(0).dtype(torch::kFloat));
  ...
  for (int i = 0; i < dst_blocks; i++) {
    ...
    for (int j = 0; j < src_blocks; j++) {
      ...
      NtsVar input_gpu =
          input_cpu
              .slice(0,
                     graph_partitions[i + j * dst_blocks]->src_range[0] -
                         graph->partition_offset[graph->partition_id],
                     graph_partitions[i + j * dst_blocks]->src_range[1] -
                         graph->partition_offset[graph->partition_id],
                     1)
              .cuda();

      weight_gpu = graph_partitions[i + j * dst_blocks]->weight.cuda();
      NtsVar src_l = graph_partitions[i + j * dst_blocks]->src.cuda();
      NtsVar dst_l = graph_partitions[i + j * dst_blocks]->dst.cuda();
      ...
      forward_on_GPU(
          input_gpu.packed_accessor<float, 2>().data(),
          output_gpu.packed_accessor<float, 2>().data(),
          weight_gpu.packed_accessor<float, 2>().data(), // data
          (uint32_t *)src_l.packed_accessor<int, 2>().data(),
          (uint32_t *)dst_l.packed_accessor<int, 2>().data(), // graph
          src_start, src_end, dst_start, dst_end,
          graph_partitions[i + j * dst_blocks]->edge_size,
          graph_partitions[i + j * dst_blocks]->batch_size, feature_size);
      kernel_time += MPI_Wtime();
    ...
    NtsVar output_slice =
        output_cpu.slice(0, graph_partitions[i]->dst_range[0],
                         graph_partitions[i]->dst_range[1], 1);
    if (i != (dst_blocks - 1)) {
      move_result_out(output_cpu.accessor<float, 2>().data() +
                          (graph_partitions[i]->dst_range[0] * feature_size),
                      output_gpu.packed_accessor<float, 2>().data(),
                      graph_partitions[i]->dst_range[0],
                      graph_partitions[i]->dst_range[1], feature_size);
    } else {
      move_result_out(output_cpu.accessor<float, 2>().data() +
                          (graph_partitions[i]->dst_range[0] * feature_size),
                      output_gpu.packed_accessor<float, 2>().data(),
...
void move_embedding_out(NtsVar &t_host, NtsVar &t_device, int src, int dst,
                        int feature_size) {
  move_result_out(t_host.accessor<float, 2>().data() + src * feature_size,
                  t_device.packed_accessor<float, 2>().data(), src, dst,
                  feature_size);
}

void propagate_forward_gpu_shard_CSC(
    Graph<Empty> *graph, NtsVar input_cpu, NtsVar &output_cpu,
  ...
  for (int i = 0; i < dst_blocks; i++) {
    ...
    for (int j = 0; j < src_blocks; j++) {
      ...
      NtsVar input_gpu =
          input_cpu
              .slice(0,
                     ...
                     1)
              .cuda();
      // if(graph->partition_id==0) std::cout<<"we fail on worker0"<<"
      // "<<graph_partitions[j]->src_range[0]<<"
      // "<<graph_partitions[j]->src_range[1]<<"
      // "<<graph->partition_offset[graph->partition_id]<<std::endl;

      weight_gpu = graph_partitions[i + j * dst_blocks]->edge_weight.cuda();
      NtsVar row_indices =
          graph_partitions[i + j * dst_blocks]->row_indices.cuda();
      NtsVar column_offset =
          graph_partitions[i + j * dst_blocks]->column_offset.cuda();
      VertexId src_start = graph_partitions[i + j * dst_blocks]->src_range[0];
      VertexId src_end = graph_partitions[i + j * dst_blocks]->src_range[1];
      VertexId dst_start = graph_partitions[i + j * dst_blocks]->dst_range[0];
      VertexId dst_end = graph_partitions[i + j * dst_blocks]->dst_range[1];
      ...
      Gather_By_Dst_From_Src(
          input_gpu.packed_accessor<float, 2>().data(),
          output_gpu.packed_accessor<float, 2>().data(),
          weight_gpu.packed_accessor<float, 2>().data(), // data
          (uint32_t *)row_indices.packed_accessor<int, 2>().data(),
          (uint32_t *)column_offset.packed_accessor<int, 2>().data(), // graph
          src_start, src_end, dst_start, dst_end,
          graph_partitions[i + j * dst_blocks]->edge_size,
          graph_partitions[i + j * dst_blocks]->batch_size, feature_size, true);
      kernel_time += get_time();
      graph->all_kernel_time += kernel_time;
      // because the src_range has quite different meaning in GPU and CPU
    }
    ...
    if (i != (dst_blocks - 1)) {
      ...
      move_result_out(output_cpu.accessor<float, 2>().data() +
                      ...
                      output_gpu.packed_accessor<float, 2>().data(),
                      graph_partitions[i]->dst_range[0],
                      graph_partitions[i]->dst_range[1], feature_size, true);
    } else {
...
void propagate_forward_gpu_shard_CSC_test(
    Graph<Empty> *graph, NtsVar input_cpu, NtsVar &output_cpu,
    std::vector<CSC_segment *> &graph_partitions, int feature_size,
    int batch_size) {
  ...
  // output_cpu.zero_();
  output_gpu =
      torch::zeros({batch_size, feature_size},
                   at::TensorOptions().device_index(0).dtype(torch::kFloat));
  double movein_time = 0;
  movein_time -= get_time();
  ...
  for (int i = 0; i < dst_blocks; i++) {
    ...
    NtsVar row_indices = graph_partitions[i]->row_indices.cuda();
    NtsVar column_offset = graph_partitions[i]->column_offset.cuda();
    ...
    Gather_By_Dst_From_Src(
        ...
        (uint32_t *)row_indices.packed_accessor<int, 2>().data(),
        (uint32_t *)column_offset.packed_accessor<int, 2>().data(), // graph
        src_start, src_end, dst_start, dst_end, graph_partitions[i]->edge_size,
        graph_partitions[i]->batch_size, feature_size, true);
    kernel_time += get_time();
    graph->all_kernel_time += kernel_time;
    // because the src_range has quite different meaning in GPU and CPU
...
void generate_weight_and_csr(Graph<Empty> *graph, VertexSubset *active,
                             VertexId *incoming_adj_index,
                             VertexId *incoming_adj_index_backward,
                             float *weight, float *weight_backward,
                             NtsVar &weight_forward_T,
                             NtsVar &weight_backward_T) {
  graph->process_vertices<ValueType>( // init  the vertex state.
      [&](VertexId vtx) {
        ...
        for (int i = graph->incoming_adj_index[0][vtx];
          ...
          weight[i] = (ValueType)std::sqrt(graph->in_degree_for_backward[vtx]) *
                      (ValueType)std::sqrt(graph->out_degree_for_backward[dst]);
        }
        for (int i = graph->incoming_adj_index_backward[0][vtx];
          ...
          weight_backward[i] =
              (ValueType)std::sqrt(graph->out_degree_for_backward[vtx]) *
              (ValueType)std::sqrt(graph->in_degree_for_backward[dst]);
        }
...
void generate_weight_and_csr_numa(Graph<Empty> *graph, VertexSubset *active,
  ...
  for (int v_i = 0; v_i < graph->vertices; v_i++) {
    ...
    for (int s_i = 0; s_i < graph->sockets; s_i++) {
      write_position += graph->incoming_adj_index[s_i][v_i + 1] -
                        graph->incoming_adj_index[s_i][v_i];
      write_position_backward +=
          graph->incoming_adj_index_backward[s_i][v_i + 1] -
          graph->incoming_adj_index_backward[s_i][v_i];
      for (int i = graph->incoming_adj_index[s_i][v_i];
           i < graph->incoming_adj_index[s_i][v_i + 1]; i++) {
        VertexId dst = graph->incoming_adj_list[s_i][i].neighbour;
        weight[write_position_edge++] =
            (ValueType)std::sqrt(graph->in_degree_for_backward[v_i]) *
            (ValueType)std::sqrt(graph->out_degree_for_backward[dst]);
      }
      for (int i = graph->incoming_adj_index_backward[s_i][v_i];
           i < graph->incoming_adj_index_backward[s_i][v_i + 1]; i++) {
        VertexId dst = graph->incoming_adj_list_backward[s_i][i].neighbour;
        weight_backward[write_position_backward_edge++] =
            (ValueType)std::sqrt(graph->out_degree_for_backward[v_i]) *
            (ValueType)std::sqrt(graph->in_degree_for_backward[dst]);
      }
    }
  }
...
/*
void inference(NtsVar tt_cpu, Graph<Empty> *graph, Embeddings<ValueType, long>
*embedding, tensorSet *pytool, GnnUnit *Gnn_v1, GnnUnit *Gnn_v2)
{
    int correct = 0;
    for (int k = 0; k < embedding->rownum; k++)
    {
        ...
        {
            if (max < tt_cpu.accessor<ValueType, 2>().data()[k * LABEL_NUMBER +
i])
            {
                max = tt_cpu.accessor<ValueType, 2>().data()[k * LABEL_NUMBER +
i]; id = i;
            }
        }
    ...
    std::cout << "\ncorrect number on training:" << correct << "\t" <<
((ValueType)correct / (ValueType)graph->vertices) << std::endl; std::cout <<
"loss at" << graph->partition_id << "is :" << pytool->loss << std::endl; int
correct_test = 0; for (int k = graph->vertices; k < NODE_NUMBER; k++)
    {
        ValueType max = -100000.0;
        long id = -1;
        NtsVar test = torch::from_blob(&(embedding->con[k].att[0]), {1,
SIZE_LAYER_1}); NtsVar final_ =
torch::relu(test.mm(Gnn_v1->W.cpu())).mm(Gnn_v2->W.cpu()).log_softmax(1); for
(int i = 0; i < LABEL_NUMBER; i++)
        {
    ...
    std::cout << "\ncorrect number on testing:" << correct_test << "\t" <<
((ValueType)correct_test / (ValueType)(NODE_NUMBER - graph->vertices)) <<
std::endl;
}
 *
 */
