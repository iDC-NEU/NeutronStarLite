...
public:
    ...
    NtsVar Propagate(int local_layer)
    {
...
//                    }
   ...
   inline void GraphPropagateBackward(NtsVar &X, NtsVar &Y, std::vector<CSC_segment_pinned *> &graph_partitions)
    ...
    template <int CURRENT_LAYER_SIZE>
    NtsVar Test_Propagate(int local_layer)
    {
        zeroCurr(); //local
        zeroNext(); //local
        zeroValue(local_layer);
    ...
    void setGradFromTensor(NtsVar new_tensor)
    {
        pre_grad = new_tensor;
    }
    NtsVar v(int local_layer)
    {
        return value_local[local_layer];
    }
    NtsVar require_grad()
...
void propagate_forward_gpu_shard(Graph<Empty> *graph, NtsVar input_cpu, NtsVar &output_cpu,
                                 std::vector<edge_list *> &graph_partitions, int feature_size)
{
    ...
    for (int i = 0; i < dst_blocks; i++)
    {
        ...
        for (int j = 0; j < src_blocks; j++)
        {
            ...
            NtsVar input_gpu = input_cpu.slice(0, graph_partitions[i + j * dst_blocks]->src_range[0] - graph->partition_offset[graph->partition_id],
        ...
        NtsVar output_slice = output_cpu.slice(0, graph_partitions[i]->dst_range[0], graph_partitions[i]->dst_range[1], 1);
        if (i != (dst_blocks - 1))
        {
...
void move_embedding_out(NtsVar &t_host, NtsVar &t_device, int src, int dst, int feature_size)
{
    move_result_out(t_host.accessor<float, 2>().data() + src * feature_size, t_device.packed_accessor<float, 2>().data(), src, dst, feature_size);
}

void propagate_forward_gpu_shard_CSC(Graph<Empty> *graph, NtsVar input_cpu, NtsVar &output_cpu,
                                     std::vector<CSC_segment *> &graph_partitions, int feature_size)
{
    ...
    for (int i = 0; i < dst_blocks; i++)
    {
        ...
        for (int j = 0; j < src_blocks; j++)
        {
            ...
            NtsVar row_indices = graph_partitions[i + j * dst_blocks]->row_indices.cuda();
            NtsVar column_offset = graph_partitions[i + j * dst_blocks]->column_offset.cuda();
            VertexId src_start = graph_partitions[i + j * dst_blocks]->src_range[0];
            VertexId src_end = graph_partitions[i + j * dst_blocks]->src_range[1];
            VertexId dst_start = graph_partitions[i + j * dst_blocks]->dst_range[0];
            VertexId dst_end = graph_partitions[i + j * dst_blocks]->dst_range[1];
...
void propagate_forward_gpu_shard_CSC_test(Graph<Empty> *graph, NtsVar input_cpu, NtsVar &output_cpu,
...
void generate_weight_and_csr(Graph<Empty> *graph, VertexSubset *active,
                             ...
                             float *weight, float *weight_backward, NtsVar &weight_forward_T, NtsVar &weight_backward_T)
{
