...
public:
  ...
  std::string VarEncode(std::string name) {
    return name.append("_")
        .append(std::to_string(rtminfo->curr_layer))
        .append("_")
        .append(std::to_string(encode_partition));
  }
  void init_blockinfo() {
    ...
    for (int l = 0; l < 2; l++) {
      ...
      for (int i = 0; i < partitions; i++) {
        blockinfo[l].max_buffer_size = std::max(
            blockinfo[l].max_buffer_size,
            blockinfo[l].block_index[i + 1] - blockinfo[l].block_index[i]);
      }
      current_partition = 0;
      for (VertexId i = 0; i < blockinfo[l].global_index.size(); i++) {
        if (i > blockinfo[l].block_index[current_partition + 1])
          current_partition++;
        blockinfo[l].local_index.push_back(
            blockinfo[l].global_index[i] -
            blockinfo[l].block_index[current_partition]);
      }

      blockinfo[l].index_gpu_buffer = (VertexId *)cudaMallocGPU(
          sizeof(VertexId) * blockinfo[l].local_index.size());
      blockinfo[l].vertex_gpu_buffer = (VertexId *)cudaMallocGPU(
          sizeof(VertexId) * blockinfo[l].local_index.size());
      move_edge_in(blockinfo[l].index_gpu_buffer,
                   &(blockinfo[l].local_index[0]), 0,
                   blockinfo[l].local_index.size(), 1, false);
      move_edge_in(blockinfo[l].vertex_gpu_buffer, &(blockinfo[l].vertex[0]), 0,
                   blockinfo[l].vertex.size(), 1, false);
  ...
  void init_message_buffer() {
    output_cpu_buffer = (float *)cudaMallocPinned(
        ((long)vertices) * gnnctx->max_layer * sizeof(float));
...
#pragma omp parallel reduction(+ : reducer)
  ...
  void flush_data_to_send_buffer_buffer_lock_free_write(
      int t_i, int f_size, VertexId key, float *value,
    ...
    memcpy(send_buffer[current_send_part_id][s_i]->data +
           ...
           &key, sizeof(VertexId));
    memcpy(send_buffer[current_send_part_id][s_i]->data +
               (sizeofM<float>(f_size)) * pos + sizeof(VertexId),
           value, f_size * sizeof(float));
  }
  ...
  void emit_buffer(VertexId vtx, t_v *buffer, int f_size) {
    ...
    memcpy(s_buffer + local_send_buffer[t_i]->count * sizeofM<float>(f_size),
           &vtx, sizeof(VertexId));
    memcpy(s_buffer + local_send_buffer[t_i]->count * sizeofM<float>(f_size) +
               sizeof(VertexId),
...
#pragma omp parallel reduction(+ : reducer)
  ...
  template <typename R, typename M>
  R process_edges_with_GPU_aggregator(
      std::function<void(VertexId, VertexAdjList<EdgeData>)> dense_signal,
      float *local_data_buffer, int feature_size, Bitmap *active,
      Bitmap *dense_selective = nullptr) {
    double stream_time = 0;
    stream_time -= MPI_Wtime();
    ...
    for (int i = 0; i < partitions; i++) {
      for (int s_i = 0; s_i < sockets; s_i++) {
        ...
        max_recv_buffer_size =
            std::max(max_recv_buffer_size,
                     (int)(partition_offset[i + 1] - partition_offset[i]));
      }
    } // init the send and receive buffer
    size_t basic_chunk = 64;
    {
...
#pragma omp parallel for
      ...
      // ALLOCATE MEMOPRY
      allocate_gpu_buffer(&gpu_memory_buffer,
                          max_recv_buffer_size * (feature_size + 1));
      ...
      for (int step = 0; step < partitions; step++) {
        ...
        for (int s_i = 0; s_i < sockets; s_i++) {
          ...
          if (used_buffer[s_i]->count > 0) {
            int recv_copy = 0;
            recv_copy -= MPI_Wtime();
            move_data_in(gpu_memory_buffer, (float *)used_buffer[s_i]->data, 0,
                         used_buffer[s_i]->count, (feature_size + 1));
            recv_copy += MPI_Wtime();
            all_recv_copy_time += recv_copy;
            // printf("call new engine %d\n",used_buffer[s_i]->count);
            int recv_kernel = 0;
            recv_kernel -= MPI_Wtime();
            aggregate_comm_result(local_data_buffer, gpu_memory_buffer,
                                  used_buffer[s_i]->count, feature_size,
                                  partition_offset[partition_id]);
            // printf("call new engine %d\n",used_buffer[s_i]->count);
            recv_kernel += MPI_Wtime();
      ...
      float *local_data_buffer) // backward
  {
    ...
    if (partition_id == 0) {
      printf("ComputeSync:layer(%d).process_local(%d).dimension(%d).reduce_"
             "comm(%d).overlap(%d)\n",
             layer_, process_local ? replication_threshold : -1,
             graph_rep[layer_].feature_size, process_local, process_overlap);
    }
    double stream_time = 0;
    stream_time -= MPI_Wtime();
    ...
    for (int i = 0; i < partitions; i++) {
      for (int s_i = 0; s_i < sockets; s_i++) {
        ...
        send_buffer[i][s_i]->count = 0;
        recv_buffer[i][s_i]->count = 0;
        // max_recv_buffer_size = std::max(max_recv_buffer_size,
        // (int)(partition_offset[i + 1] - partition_offset[i]));
      }
    } // init the send and receive buffer
    ...
    for (int i = 0; i < graph_partitions.size(); i++) {
      max_edge_size =
          std::max(max_edge_size, (size_t)(graph_partitions[i]->edge_size));
      if (rtminfo->forward)
        max_dstrange =
            std::max(max_dstrange, (size_t)(graph_partitions[i]->dst_range[1] -
                                            graph_partitions[i]->dst_range[0]));
      else
        max_dstrange =
            std::max(max_dstrange, (size_t)(graph_partitions[i]->src_range[1] -
                                            graph_partitions[i]->src_range[0]));
    };

    weight_gpu_intergate =
        (float *)cudaMallocGPU(max_edge_size * sizeof(float));
    row_indices_intergate =
        (VertexId *)cudaMallocGPU(max_edge_size * sizeof(VertexId));
    column_offset_intergate =
        (VertexId *)cudaMallocGPU((max_dstrange + 1) * sizeof(VertexId));
    ...
    if (process_local) {
      index_gpu_output = (VertexId *)cudaMallocGPU(vertices * sizeof(VertexId));
      index_gpu_input = (VertexId *)cudaMallocGPU(vertices * sizeof(VertexId));
      graph_rep[layer_].rep_feature_gpu_buffer = (float *)cudaMallocGPU(
          ((long)sizeof(float)) * graph_rep[layer_].rep_node_size *
              graph_rep[layer_].feature_size +
          1);
      src = (VertexId *)cudaMallocGPU(
          sizeof(VertexId) * graph_rep[layer_].rep_edge_size + 1);
      dst = (VertexId *)cudaMallocGPU(
          sizeof(VertexId) * graph_rep[layer_].rep_edge_size + 1);
      weight = (float *)cudaMallocGPU(
          sizeof(float) * graph_rep[layer_].rep_edge_size + 1);
      //	printf("initialize send buffer finished %d\n", partition_id);
    }
    size_t basic_chunk = 64;
    {
      ...
      if (process_overlap) {
        ...
        if (rtminfo->forward) {
          forward_gpu_CSC_partition(
              input_gpu_or_cpu, graph_partitions, feature_size,
              (current_send_part_id + 1) % partitions, cuda_stream);
        } else {
        ...
        if (current_send_part_id != partition_id) {
          ...
          if (process_overlap) {
            ...
            if (process_local) {
              ...
              // MPI_Barrier(MPI_COMM_WORLD);
              forward_gpu_local(graph_rep, local_data_buffer, cuda_stream, src,
                                dst, weight, index_gpu_input, index_gpu_output,
                                feature_size, layer_);
              // cuda_stream->CUDA_DEVICE_SYNCHRONIZE();
            }
            replication_time += MPI_Wtime();
...
#pragma omp parallel for
        ...
        if (i != partition_id) {
          //           for (int s_i=0;s_i<sockets;s_i++) {
          //            send_buffer[i][s_i]->count=(partition_offset[i+1] -
          //            partition_offset[i]);
          //          }
          send_queue[send_queue_size] = i;
          send_queue_mutex.lock();
      ...
      for (int step = 0; step < partitions; step++) {
        ...
        for (int s_i = 0; s_i < sockets; s_i++) {
          ...
          if (used_buffer[s_i]->count > 0) {
            // printf("%ld %ld %ld\n", used_buffer[s_i]->count,
            // max_recv_buffer_size, max_recv_buffer_size * (feature_size + 1));
            int recv_copy = 0;
            recv_copy -= MPI_Wtime();
            cuda_stream->move_data_in(
                gpu_memory_buffer, (float *)used_buffer[s_i]->data, 0,
                used_buffer[s_i]->count, (feature_size + 1), false);
            // cuda_stream->CUDA_DEVICE_SYNCHRONIZE();

            for (int i = 0; i < used_buffer[s_i]->count; i++) {
              VertexId *S = (VertexId *)used_buffer[s_i]->data;
              if (S[i * (feature_size + 1)] < partition_offset[partition_id] ||
                  S[i * (feature_size + 1)] >=
                      partition_offset[partition_id + 1])
            ...
            cuda_stream->aggregate_comm_result_debug(
                local_data_buffer, gpu_memory_buffer, used_buffer[s_i]->count,
                feature_size, partition_offset[partition_id],
                partition_offset[partition_id + 1], false);
            // cuda_stream->CUDA_DEVICE_SYNCHRONIZE();
            recv_kernel += MPI_Wtime();
            all_recv_kernel_time += recv_kernel;
      ...
      if (!process_overlap) {
        ...
        if (process_local) {
          ...
          // layer_); MPI_Barrier(MPI_COMM_WORLD);
          forward_gpu_local(graph_rep, local_data_buffer, cuda_stream, src, dst,
                            weight, index_gpu_input, index_gpu_output,
                            feature_size, layer_);
          cuda_stream->CUDA_DEVICE_SYNCHRONIZE();
        }
        replication_time += MPI_Wtime();
  ...
  R compute_sync_lite(
    ...
    if (partition_id == 0) {
      printf("ComputeSyncLite:layer(%d).process_local(%d).dimension(%d).reduce_"
             "comm(%d).overlap(%d)\n",
             layer_, process_local ? replication_threshold : -1,
             graph_rep[layer_].feature_size, process_local, process_overlap);
    }
...
#pragma omp parallel
        {
          ...
          while (true) {
            ...
            for (VertexId p_v_i = begin_p_v_i; p_v_i < end_p_v_i; p_v_i++) {
              ...
              // incoming_adj_list[s_i] +
              // compressed_incoming_adj_index[s_i][p_v_i + 1].index));
              flush_data_to_send_buffer_buffer_lock_free_write(
                  thread_id, feature_size, v_i,
                  output_cpu_buffer + v_i * feature_size,
                  message_write_offset[layer_][v_i]);
            }
          }
          //  printf("Send initial has been finished %d\n", partition_id);
          thread_state[thread_id]->status = STEALING;
        ...
        flush_data_to_send_buffer_buffer_lock_free_init(
            message_amount[layer_][current_send_part_id]);
        if (message_amount[layer_][current_send_part_id] >
            (partition_offset[current_send_part_id + 1] -
             partition_offset[current_send_part_id]))
          printf("DEASTER\n");
        if (i != partition_id) {
  ...
  R sync_compute(NtsVar &input_gpu_or_cpu,
    ...
    if (partition_id == 0) {
      printf("SyncComputeLite:layer(%d).process_local(%d).dimension(%d).reduce_"
             "comm(%d).overlap(%d)\n",
             layer_, process_local ? replication_threshold : -1,
             graph_rep[layer_].feature_size, process_local, process_overlap);
    }
    ...
    for (int t_i = 0; t_i < threads; t_i++) {
      //  printf("alloc local send buffer
    ...
    for (int i = 0; i < partitions; i++) {
      for (int s_i = 0; s_i < sockets; s_i++) {
        recv_buffer[i][s_i]->resize_pinned(
            sizeofM<M>(feature_size) *
            (partition_offset[i + 1] - partition_offset[i]) * sockets);
        send_buffer[i][s_i]->resize_pinned(sizeofM<M>(feature_size) *
                                           owned_vertices * sockets);
        send_buffer[i][s_i]->count = 0;
        recv_buffer[i][s_i]->count = 0;
        max_recv_buffer_size = std::max(
            max_recv_buffer_size,
            (long)(partition_offset[i + 1] - partition_offset[i]) * sockets);
        max_partition_size =
    ...
    allocate_gpu_buffer(&gpu_memory_buffer,
                        max_recv_buffer_size * (feature_size + 1));
    allocate_gpu_buffer(&gpu_input_buffer,
                        max_partition_size * (feature_size + 1));
    // if (gpu_memory_buffer == NULL)
    // {
    //   printf("something wrong\n");
    // }
...
#pragma omp parallel for
      ...
      for (int step = 0; step < partitions; step++) {
        ...
        for (int s_i = 0; s_i < sockets; s_i++) {
          ...
          cuda_stream->move_data_in(
              gpu_memory_buffer, (float *)used_buffer[s_i]->data, 0,
              used_buffer[s_i]->count, (feature_size + 1), false);
          zero_buffer(gpu_input_buffer,
                      (partition_offset[i + 1] - partition_offset[i]) *
                          (feature_size));
          // printf("partition offset %d
          // %d\n",partition_offset[i],partition_offset[i+1]);
          cuda_stream->deSerializeToGPU(gpu_input_buffer, gpu_memory_buffer,
                                        used_buffer[s_i]->count, feature_size,
                                        partition_offset[i],
                                        partition_offset[i + 1], false);
          // zero_buffer(local_data_buffer, (partition_offset[partition_id + 1]
          ...
          forward_gpu_CSC_partition_from_buffer(
              gpu_input_buffer, local_data_buffer, graph_partitions,
              feature_size, i, cuda_stream);
          //   cuda_stream->CUDA_DEVICE_SYNCHRONIZE();
          //}
        }
      }
  ...
  // process edges
  template <typename R, typename M>
  R sync_compute_edge_computation(
      NtsVar &input_origin, // forward computation
      std::vector<CSC_segment_pinned *> &graph_partitions,
      std::function<void(VertexId)> sparse_signal,
      std::function<NtsVar(NtsVar &)> PreComputation,
      std::function<NtsVar(NtsVar &, NtsVar &, NtsVar &, NtsVar &,
                           NtsScheduler *nts)>
          EdgeComputation,
      NtsVar &output) {
...
#pragma omp parallel for
      ...
      for (int step = 0; step < partitions; step++) {
        ...
        for (int s_i = 0; s_i < sockets; s_i++) {
          ...
          cuda_stream->deSerializeToGPU(gpu_input_buffer, gpu_memory_buffer,
                                        ...
                                        partition_offset[i],
                                        partition_offset[i + 1], false);
          // torch::from_blob(graph->in_degree +
          // graph->partition_offset[graph->partition_id], {embedding->rownum,
          // 1});
          NtsVar mirror_inputs = Nts->NewKeyTensor(
              gpu_input_buffer,
              {partition_offset[i + 1] - partition_offset[i],
               feature_size}); //,at::TensorOptions().requires_grad(true).device_index(0).dtype(torch::kFloat));

          Nts->InitBlock(graph_partitions[i], rtminfo,
                         gnnctx->layer_size[rtminfo->curr_layer],
                         gnnctx->layer_size[rtminfo->curr_layer + 1],
                         current_recv_part_id, layer_, cuda_stream);
          encode_partition =
              i; // must specify the encode_partition, or the message will be
                 // flushed even though call encode function.
          NtsVar mirror_inputs_transferred;
          NtsVar message =
              EdgeComputation(mirror_inputs, mirror_inputs_transferred,
                              input_origin, input_transferred, Nts);
  ...
  template <typename R, typename M>
  R compute_sync_edge_computation(
      NtsVar &input_grad, NtsVar &dst_input,
      std::vector<CSC_segment_pinned *> &graph_partitions,
      std::function<void(VertexId, VertexAdjList<EdgeData>)> dense_signal,
      std::function<NtsVar(NtsVar &)> PreComputation,
      std::function<NtsVar(NtsVar &, NtsVar &, NtsVar &, NtsVar &,
      ...
      if (process_overlap) // pipeline
      {
        ...
        for (int step = 0; step < partitions; step++) {
          ...
          Nts->InitBlock(graph_partitions[current_send_part_id], rtminfo,
                         gnnctx->layer_size[rtminfo->curr_layer],
                         gnnctx->layer_size[rtminfo->curr_layer + 1],
                         current_send_part_id, layer_, cuda_stream);
          NtsVar src_input_transferred; // mark
          NtsVar src_input;
          NtsVar message = EdgeForward(src_input, src_input_transferred,
                                       dst_input, dst_input_transferred, Nts);
          ...
          NtsVar src_grad =
              EdgeBackward(message_grad, src_inter_grad, Nts); //(2,3)->1
...
#pragma omp parallel for
  ...
  template <typename R, typename M>
  R compute_and_sync_with_GPU_aggregator_overlap(
      NtsVar &input_gpu_or_cpu,
      std::vector<CSC_segment_pinned *> &graph_partitions,
      std::function<void(VertexId, VertexAdjList<EdgeData>)> dense_signal,
      float *local_data_buffer, int feature_size, bool process_local = false,
      int layer_ = 0, bool process_overlap = false) {

    process_local = process_local && (graph_rep[layer_].rep_edge_size > 0);

    if (partition_id == 0) {
      printf("layer(%d).process_local(%d).dimension(%d).reduce_comm(%d)."
             "overlap(%d)\n",
             layer_, process_local ? replication_threshold : -1,
             graph_rep[layer_].feature_size, process_local, process_overlap);
    }
    ...
    for (int i = 0; i < partitions; i++) {
      for (int s_i = 0; s_i < sockets; s_i++) {
        recv_buffer[i][s_i]->resize_pinned(sizeof(MsgUnit<M>) * owned_vertices *
                                           sockets);
        send_buffer[i][s_i]->resize_pinned(
            sizeof(MsgUnit<M>) *
            (partition_offset[i + 1] - partition_offset[i]) * sockets);
...
#pragma omp parallel for
      ...
      for (int step = 0; step < partitions; step++) {
        ...
        for (int s_i = 0; s_i < sockets; s_i++) {
          ...
          if (used_buffer[s_i]->count > 0) {
            ...
            int recv_kernel = 0;
            recv_kernel -= MPI_Wtime();
            cuda_stream->aggregate_comm_result(
                local_data_buffer, gpu_memory_buffer, used_buffer[s_i]->count,
                feature_size, partition_offset[partition_id], false);
            // cuda_stream->CUDA_DEVICE_SYNCHRONIZE();
            recv_kernel += MPI_Wtime();
            all_recv_kernel_time += recv_kernel;
          }
  ...
  void forward_gpu_local(graph_replication *graph_rep, float *local_data_buffer,
                         Cuda_Stream *cuda_stream, VertexId *src, VertexId *dst,
                         float *weight, VertexId *index, VertexId *index_output,
                         int feature_size, int layer_) {
    // if(partition_id==0)
    ...
    cuda_stream->move_data_in(graph_rep[layer].rep_feature_gpu_buffer,
                              graph_rep[layer].rep_feature, 0,
                              graph_rep[layer].rep_node_size,
                              graph_rep[layer].feature_size, false);

    cuda_stream->move_edge_in(index, graph_rep[layer].src_map, 0, vertices, 1,
                              false);
    cuda_stream->move_edge_in(index_output, graph_rep[layer].dst_map, 0,
                              vertices, 1, false);
    cuda_stream->move_edge_in(src, graph_rep[layer].src_rep, 0,
                              graph_rep[layer].rep_edge_size, 1, false);
    cuda_stream->move_edge_in(dst, graph_rep[layer].dst_rep, 0,
                              graph_rep[layer].rep_edge_size, 1, false);
    cuda_stream->move_data_in(weight, graph_rep[layer].weight_rep, 0,
                              graph_rep[layer].rep_edge_size, 1, false);
    ...
    cuda_stream->process_local_inter(
        graph_rep[layer].output_buffer_gpu,
        graph_rep[layer].rep_feature_gpu_buffer, src, dst, index, index_output,
        weight, partition_offset[partition_id],
        partition_offset[partition_id + 1], graph_rep[layer].feature_size,
        graph_rep[layer].rep_edge_size, rep_output_size, false);
  ...
  void forward_gpu_CSC_partition_from_buffer(
      ...
      int current_send_partition_id, Cuda_Stream *cuda_stream,
    ...
    int vertex_range =
        partition_offset[partition_id + 1] - partition_offset[partition_id];
    int dst_range = graph_partitions[current_send_partition_id]->dst_range[1] -
                    graph_partitions[current_send_partition_id]->dst_range[0];

    cuda_stream->move_data_in(
        weight_gpu_intergate,
        graph_partitions[current_send_partition_id]->edge_weight, 0,
        graph_partitions[current_send_partition_id]->edge_size, 1, false);
    cuda_stream->move_edge_in(
        row_indices_intergate,
        (VertexId *)(graph_partitions[current_send_partition_id]->row_indices),
        0, graph_partitions[current_send_partition_id]->edge_size, 1, false);
    cuda_stream->move_edge_in(
        column_offset_intergate,
        graph_partitions[current_send_partition_id]->column_offset, 0,
        dst_range + 1, 1, false);
    ...
    cuda_stream->Gather_By_Dst_From_Src(
        ...
        graph_partitions[current_send_partition_id]->batch_size, feature_size,
        false);
    // cuda_stream->CUDA_DEVICE_SYNCHRONIZE();
    kernel_time += MPI_Wtime();
    ...
    //    cuda_stream->move_result_out(output_cpu_buffer +
    //    (graph_partitions[current_send_partition_id]->dst_range[0] *
    //    feature_size),
    //                                 output_gpu_buffered.packed_accessor<float,
    //                                 2>().data(),
    //                                 graph_partitions[current_send_partition_id]->dst_range[0],
    //                                 graph_partitions[current_send_partition_id]->dst_range[1],
    //                                 feature_size, false);
  ...
  void
  forward_gpu_CSC_partition(NtsVar input_gpu_or_cpu,
                            std::vector<CSC_segment_pinned *> &graph_partitions,
                            int feature_size, int current_send_partition_id,
    ...
    kernel_time -= MPI_Wtime();
    cuda_stream->Gather_By_Dst_From_Src(
        Nts->getWritableBuffer(input_gpu_or_cpu),
    ...
    cuda_stream->move_result_out(
        output_cpu_buffer +
            (graph_partitions[current_send_partition_id]->dst_range[0] *
             feature_size),
        Nts->getWritableBuffer(output_gpu_buffered),
        graph_partitions[current_send_partition_id]->dst_range[0],
        graph_partitions[current_send_partition_id]->dst_range[1], feature_size,
        false);
  ...
  void backward_gpu_CSR_partition(
    ...
    int vertex_range =
        graph_partitions[current_send_partition_id]->dst_range[1] -
        graph_partitions[current_send_partition_id]->dst_range[0];
    int dst_range = graph_partitions[current_send_partition_id]->src_range[1] -
                    graph_partitions[current_send_partition_id]->src_range[0];

    cuda_stream->move_data_in(
        weight_gpu_intergate,
        graph_partitions[current_send_partition_id]->edge_weight_backward, 0,
        graph_partitions[current_send_partition_id]->edge_size, 1, false);
    cuda_stream->move_edge_in(
        row_indices_intergate,
        (VertexId *)(graph_partitions[current_send_partition_id]
                         ->column_indices),
        0, graph_partitions[current_send_partition_id]->edge_size, 1, false);
    cuda_stream->move_edge_in(
        column_offset_intergate,
        graph_partitions[current_send_partition_id]->row_offset, 0,
        dst_range + 1, 1, false);
    ...
    cuda_stream->Gather_By_Dst_From_Src(
        input_gpu_or_cpu.packed_accessor<float, 2>().data(),
    ...
    cuda_stream->move_result_out(
        ...
        output_gpu_buffered.packed_accessor<float, 2>().data(),
        graph_partitions[current_send_partition_id]->src_range[0],
        graph_partitions[current_send_partition_id]->src_range[1], feature_size,
        false);
            ...
            while (thread_state[t_i]->status != STEALING) {
              ...
              for (VertexId p_v_i = begin_p_v_i; p_v_i < end_p_v_i; p_v_i++) {
                VertexId v_i =
                    compressed_incoming_adj_index_backward[s_i][p_v_i].vertex;
                // VertexId v_i =
                // compressed_outgoing_adj_index[s_i][p_v_i].vertex;
                //  dense_signal(v_i,
                //  VertexAdjList<EdgeData>(incoming_adj_list[s_i] +
                //  compressed_incoming_adj_index[s_i][p_v_i].index,
                //  incoming_adj_list[s_i] +
...
#pragma omp parallel reduction(+ : reducer)
  ...
  void generate_backward_structure() {
    ...
    for (int i = 0; i < sockets; i++) {
      ...
      numa_free(outgoing_adj_list[i],
                sizeof(AdjUnit<EdgeData>) * outgoing_edges[i]);
      numa_free(compressed_outgoing_adj_index[i],
                sizeof(CompressedAdjIndexUnit) *
  ...
  void generate_COO(VertexSubset *active) {
    ...
    for (int i_s; i_s < sockets; i_s++) {
      for (VertexId vtx = 0; vtx < vertices; vtx++) {
        ...
        if (incoming_adj_index_backward[i_s][vtx + 1] == 0)
          incoming_adj_index_backward[i_s][vtx + 1] =
              incoming_adj_index_backward[i_s][vtx];
      }
    }
    for (VertexId vtx = 0; vtx < vertices; vtx++) {
    ...
    for (int k = 0; k < sockets; k++)
      for (VertexId vtx = 0; vtx < vertices; vtx++) {
        for (int i = incoming_adj_index[k][vtx];
             i < incoming_adj_index[k][vtx + 1]; i++) {
          _graph_cpu->dstList[write_position] = vtx;
          _graph_cpu->srcList[write_position++] =
              incoming_adj_list[k][i].neighbour;
        }
      }
    if (partition_id == 0)
      printf("GNNmini::Preprocessing[Generate Edges]\n");
  }
  void reorder_COO(int batch_size) { // replication
    graph_shard.clear();

    VertexId edge_size = (VertexId)incoming_adj_index[0][vertices];
    int blocks_v_num = batch_size;
    int src_blocks = owned_vertices / blocks_v_num +
                     (int)((owned_vertices % blocks_v_num) > 0);
    int dst_blocks =
        vertices / blocks_v_num + (int)((vertices % blocks_v_num) > 0);
    for (int i = 0; i < src_blocks * dst_blocks; i++) {
      graph_shard.push_back(new COOChunk());
      graph_shard[i]->src_range[0] =
          i / dst_blocks * blocks_v_num + partition_offset[partition_id];
      graph_shard[i]->src_range[1] =
          std::min((1 + i / dst_blocks) * blocks_v_num, (int)owned_vertices) +
          partition_offset[partition_id];
      graph_shard[i]->dst_range[0] = i % dst_blocks * blocks_v_num;
      graph_shard[i]->dst_range[1] =
          std::min((1 + i % dst_blocks) * blocks_v_num, (int)vertices);
      // if(partition_id==1)std::cout<<"test in reorder "<<"
      // "<<batch_size<<graph_shard[i]->src_range[0]<<"
      // "<<graph_shard[i]->src_range[1]<<std::endl;
    }
    // std::cout<<this->partition_id<<" "<<batch_size<<" "<<src_blocks<<"
    // "<<dst_blocks<<std::endl;
    // std::cout<<_graph_cpu->srcList[0]<<"????"<<partition_offset[partition_id+1]<<"
    // "<<this->edges<<std::endl;
    ...
    for (int i = 0; i < edge_size; i++) {
      ...
      // }
      int src_bucket =
          (_graph_cpu->srcList[i] - partition_offset[partition_id]) /
          blocks_v_num;
      int dst_bucket = (_graph_cpu->dstList[i]) / blocks_v_num;
      // if(partition_id==1)
      // std::cout<<_graph_cpu->srcList[i]<<"
      // "<<partition_offset[partition_id]<<std::endl;
    ...
    for (int i = 0; i < edge_size; i++) {
      ...
      graph_shard[bucket_s * dst_blocks + bucket_d]->dst_delta[offset] =
          destination;
    }
  }
  ...
  void reorder_COO_W2partition(int batch_size) { // replication
    graph_shard.clear();
    ...
    for (int i = 0; i < (dst_blocks); i++) {
      ...
      graph_shard[i]->dst_range[1] =
          std::min((i + 1) * batch_size, (int)(vertices));
      //      if(partition_id==0)
      //      printf("here is the problem %d %d\n",graph_shard[i]->dst_range[0],
      //      graph_shard[i]->dst_range[1]);
    }
    for (int i = 0; i < edge_size; i++) {
  ...
  void load_replicate3(std::vector<int> layer_size) {
    ...
    process_edges<int, int>( // For EACH Vertex Processing
        [&](VertexId dst, VertexAdjList<Empty> incoming_adj) { // pull
          ...
          if (red > 0 && (dst < partition_offset[partition_id] ||
                          dst >= partition_offset[partition_id + 1]))
    ...
    MPI_Allreduce(&pre_reduce_com, &minnodes, 1, vid_t, MPI_MIN,
                  MPI_COMM_WORLD);
    ...
    for (int i = 0; i < partitions; i++) {
      MPI_Allreduce(MPI_IN_PLACE, tag[i]->data, (WORD_OFFSET(vertices) + 1),
                    l_vid_t, MPI_SUM, MPI_COMM_WORLD);
    }
    ...
    for (int i = partition_offset[partition_id];
         i < partition_offset[partition_id + 1]; i++) {
      ...
      for (int k = 0; k < partitions; k++) {
        if (data[k].size() <= replication_threshold) {
          ...
          for (int l = 0; l < data[k].size(); l++) {
            tmp_count_edge2 +=
                column_offset[data[k][l] + 1] - column_offset[data[k][l]];
          }
          // float cost = (float)(tmp_count_edge2)*2; //layer_size[0] /
          // layer_size[1]; if (cost <= replication_threshold && cost > 0)
          if ((tmp_count_edge2 * 2) <= replication_threshold &&
              (tmp_count_edge2 * 2) > 0) {
            tag2[k]->set_bit(i);
            for (int l = 0; l < data[k].size(); l++) {
    ...
    for (int i = 0; i < vertices; i++) {
      if (intermediate->get_bit(i)) {
        ...
        for (int i_i = column_offset[i]; i_i < column_offset[i + 1]; i_i++) {
          ...
          if (partition_offset[partition_id] > src_inter ||
              partition_offset[partition_id + 1] <= src_inter) {
            tmp_count_edge2++;
    ...
    for (int i = 0; i < partitions; i++) {
      // printf("comm tag%d\n",i);
      MPI_Allreduce(MPI_IN_PLACE, tag2[i]->data, (WORD_OFFSET(vertices) + 1),
                    l_vid_t, MPI_SUM, MPI_COMM_WORLD);
      //  if(i!=partition_id){
      //   tag2[i]->~Bitmap();
      //   }
    ...
    graph_rep[0].dst_rep = (VertexId *)cudaMallocPinned(
        sizeof(VertexId) * graph_rep[0].rep_edge_size + 1);
    graph_rep[0].src_rep = (VertexId *)cudaMallocPinned(
        sizeof(VertexId) * graph_rep[0].rep_edge_size + 1);
    graph_rep[0].weight_rep = (float *)cudaMallocPinned(
        sizeof(float) * graph_rep[0].rep_edge_size + 1);
    graph_rep[0].dst_rep_gpu =
        (VertexId *)getDevicePointer((void *)graph_rep[0].dst_rep);
    graph_rep[0].src_rep_gpu =
        (VertexId *)getDevicePointer((void *)graph_rep[0].src_rep);
    graph_rep[0].weight_rep_gpu =
        (float *)getDevicePointer((void *)graph_rep[0].weight_rep);

    graph_rep[1].rep_edge_size = count_edge3;
    graph_rep[1].dst_rep = (VertexId *)cudaMallocPinned(
        sizeof(VertexId) * graph_rep[1].rep_edge_size + 1);
    graph_rep[1].src_rep = (VertexId *)cudaMallocPinned(
        sizeof(VertexId) * graph_rep[1].rep_edge_size + 1);
    graph_rep[1].weight_rep = (float *)cudaMallocPinned(
        sizeof(float) * graph_rep[1].rep_edge_size + 1);
    graph_rep[1].dst_rep_gpu =
        (VertexId *)getDevicePointer((void *)graph_rep[1].dst_rep);
    graph_rep[1].src_rep_gpu =
        (VertexId *)getDevicePointer((void *)graph_rep[1].src_rep);
    graph_rep[1].weight_rep_gpu =
        (float *)getDevicePointer((void *)graph_rep[1].weight_rep);
    memcpy(graph_rep[1].dst_rep, graph_rep[1].dst_rep_vec.data(),
           graph_rep[1].dst_rep_vec.size() * sizeof(VertexId));
    memcpy(graph_rep[1].src_rep, graph_rep[1].src_rep_vec.data(),
           graph_rep[1].src_rep_vec.size() * sizeof(VertexId));
    int start_index = 0;
    int start_index2 = 0;
      ...
      for (int k = 0; k < partitions; k++) {
        if (data[k].size() <= replication_threshold) {
          ...
          for (int l = 0; l < data[k].size(); l++) {
            ...
            graph_rep[0].weight_rep[start_index] =
                std::sqrt(indgr1) * std::sqrt(indgr2);
            start_index++;
          }
        }
        data[k].clear();
    ...
    for (int i = 0; i < vertices; i++) {
      if (intermediate->get_bit(i)) {
        ...
        for (int i_i = column_offset[dst_inter];
             i_i < column_offset[dst_inter + 1]; i_i++) {
          VertexId src_inter = row_indices[i_i];
          if (partition_offset[partition_id] > src_inter ||
              partition_offset[partition_id + 1] <= src_inter) {
            ...
            int indgr1 =
                column_offset[dst_inter + 1] - column_offset[dst_inter];
            int indgr2 =
                column_offset[src_inter + 1] - column_offset[src_inter];
            graph_rep[0].weight_rep[start_index] =
                std::sqrt(indgr1) * std::sqrt(indgr2);
            start_index++;
            HasRepVtx[1]->set_bit(src_inter);
    ...
    graph_rep[0].dst_map =
        (VertexId *)cudaMallocPinned(sizeof(VertexId) * vertices);
    graph_rep[1].dst_map =
        (VertexId *)cudaMallocPinned(sizeof(VertexId) * vertices);
    ...
    graph_rep[0].output_buffer_gpu = (float *)cudaMallocGPU(
        ((long)sizeof(float)) * graph_rep[0].output_size *
            graph_rep[0].feature_size +
        1);
    graph_rep[1].output_buffer_gpu = (float *)cudaMallocGPU(
        ((long)sizeof(float)) * graph_rep[1].output_size *
            graph_rep[1].feature_size +
        1);
    tmp1->~Bitmap();
    tmp2->~Bitmap();
    graph_rep[0].src_map =
    ...
    graph_rep[1].rep_feature = (float *)cudaMallocPinned(
        sizeof(float) * graph_rep[1].feature_size * graph_rep[1].rep_node_size +
        1);
    graph_rep[0].rep_feature = (float *)cudaMallocPinned(
        sizeof(float) * graph_rep[0].feature_size * graph_rep[0].rep_node_size +
        1);

    int errorno = 0;
    // printf("%d %d %d\n", partition_id, graph_rep[1].src_rep[2],
    // graph_rep[1].dst_rep[2]);
    for (int i = 0; i < graph_rep[1].rep_edge_size; i++) {
    ...
    //    printf("DEBUG Info 1:(%d %d) 2:(%d %d) 3:(%d %d) 4:(%d %d) 5:(%d %d)
    //    6:(%d %d %d)\n",
    ...
    MPI_Allreduce(&reduce_comm[0], &global_comm, 1, vid_t, MPI_SUM,
                  MPI_COMM_WORLD);
    MPI_Allreduce(&reduce_comm[1], &global_comm2, 1, vid_t, MPI_SUM,
                  MPI_COMM_WORLD);
    MPI_Allreduce(&graph_rep[0].rep_edge_size, &global_repedge, 1, vid_t,
                  MPI_SUM, MPI_COMM_WORLD);
    MPI_Allreduce(&graph_rep[0].rep_node_size, &global_repnode, 1, vid_t,
                  MPI_SUM, MPI_COMM_WORLD);
    MPI_Allreduce(&graph_rep[1].rep_edge_size, &global_repedge2, 1, vid_t,
                  MPI_SUM, MPI_COMM_WORLD);
    MPI_Allreduce(&graph_rep[1].rep_node_size, &global_repnode2, 1, vid_t,
                  MPI_SUM, MPI_COMM_WORLD);
    MPI_Allreduce(&all_comm_v, &global_all_comm, 1, vid_t, MPI_SUM,
                  MPI_COMM_WORLD);
    start_time += get_time();
    ...
    /*   printf("%d{|V|:%d |E|:%d}:{Layer1 rep edge:%d  noeds: %d comm %d}
       {Layer2 rep edge:%d  noeds: %d comm %d} {%d %d %d}{%d %d %d} {g_comm: %d
       local_comm: %d} ||in %f(s)\n", replication_threshold, owned_vertices,
       column_offset[local_d] - column_offset[local_s], global_repedge /
       partitions, global_repnode / partitions, global_comm / partitions,
           global_repedge2 / partitions, global_repnode2 / partitions,
       global_comm2 / partitions, graph_rep[0].rep_edge_size,
       graph_rep[0].rep_node_size, reduce_comm[0], graph_rep[1].rep_edge_size,
       graph_rep[1].rep_node_size, reduce_comm[1], global_all_comm / partitions,
  ...
  void print_info() {
    ...
    MPI_Allreduce(MPI_IN_PLACE, local_e_num, partitions, vid_t, MPI_SUM,
                  MPI_COMM_WORLD);
    MPI_Allreduce(MPI_IN_PLACE, local_v_num, partitions, vid_t, MPI_SUM,
                  MPI_COMM_WORLD);
    MPI_Allreduce(MPI_IN_PLACE, reduce_comm0, partitions, vid_t, MPI_SUM,
                  MPI_COMM_WORLD);
    MPI_Allreduce(MPI_IN_PLACE, reduce_comm1, partitions, vid_t, MPI_SUM,
                  MPI_COMM_WORLD);
    MPI_Allreduce(MPI_IN_PLACE, rep_edge_0, partitions, vid_t, MPI_SUM,
                  MPI_COMM_WORLD);
    MPI_Allreduce(MPI_IN_PLACE, rep_edge_1, partitions, vid_t, MPI_SUM,
                  MPI_COMM_WORLD);
    MPI_Allreduce(MPI_IN_PLACE, rep_node_0, partitions, vid_t, MPI_SUM,
                  MPI_COMM_WORLD);
    MPI_Allreduce(MPI_IN_PLACE, rep_node_1, partitions, vid_t, MPI_SUM,
                  MPI_COMM_WORLD);
    MPI_Allreduce(MPI_IN_PLACE, out_node, partitions, vid_t, MPI_SUM,
                  MPI_COMM_WORLD);
    ...
    if (partition_id == 0) {
      ...
      printf("GNNmini::Vertices:[%d],Edges:[%d], RepThreshold:[%d], Layers: ",
             vertices, edges, config->repthreshold);
