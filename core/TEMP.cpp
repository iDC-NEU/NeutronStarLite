...
#pragma omp parallel reduction(+ : reducer)
  ...
  void flush_local_send_buffer_buffer(int t_i) {
    ...
    if (local_send_buffer[t_i]->count != 0)
      memcpy(send_buffer[current_send_part_id][s_i]->data +
                 (sizeof(MsgUnit_buffer) + message_unit_size) * pos,
             local_send_buffer[t_i]->data,
             (sizeof(MsgUnit_buffer) + message_unit_size) *
                 local_send_buffer[t_i]->count);
    local_send_buffer[t_i]->count = 0;
  }
  ...
  template <typename t_v> void emit_buffer(VertexId vtx, t_v *buffer) {
    printf("int\n");
    int t_i = omp_get_thread_num();
    printf("%d\t%d\t%d\n", 1, local_send_buffer[t_i]->count,
           local_send_buffer[t_i]->capacity);

    *(local_send_buffer[t_i]->getMsgUnit(local_send_buffer[t_i]->count,
                                         message_unit_size)) = vtx;
    printf("%d\t%d\t%d\n", t_i, local_send_buffer[t_i]->count,
           local_send_buffer_limit);
    local_send_buffer[t_i]->set_Msg_Data<t_v>(local_send_buffer[t_i]->count,
                                              message_unit_size, buffer);
  ...
  //         sockets ); send_buffer[i][s_i]->resize( sizeof(MsgUnit<M>) *
  ...
  //           for (int s_i=0;s_i<sockets;s_i++) {
  //             MPI_Send(send_buffer[i][s_i]->data, sizeof(MsgUnit<M>) *
  //             send_buffer[i][s_i]->count, MPI_CHAR, i, PassMessage,
  //             MPI_COMM_WORLD);
  //           }
  //         }
  //       });
  ...
  //     MPI_Datatype dt = get_mpi_data_type<R>();
  //     MPI_Allreduce(&reducer, &global_reducer, 1, dt, MPI_SUM,
  //     MPI_COMM_WORLD); stream_time += MPI_Wtime(); #ifdef
  //     PRINT_DEBUG_MESSAGES if (partition_id==0) {
...
#pragma omp parallel reduction(+ : reducer)
  ...
  void generate_backward_structure() {
    ...
    //         int
    //         a1=compressed_incoming_adj_index_backward[0][b].index-compressed_incoming_adj_index_backward[0][b-1].index;//incoming_adj_index_backward[0][b+1]-incoming_adj_index_backward[0][b];
    ...
    //
    //         for(int
    //         i=compressed_incoming_adj_index_backward[0][b-1].index;i<compressed_incoming_adj_index_backward[0][b].index;i++){
  ...
  void generate_partition_for_device() {
    ...
    printf("vertice chunk %d\town chunk %d\t\tvertice chunk %d\town chunk %d\n",
           csc_partition_size[0], csc_partition_size[1], csr_partition_size[0],
           csr_partition_size[1]);
    ...
    while (read_bytes <
      ...
      for (EdgeId e_i = 0; e_i < curr_read_edges; e_i++) {
        ...
        VertexId dst = read_edge_buffer[e_i].dst;
        int tmp = 0;
        if (partition_offset[partition_id] < src &&
            src < partition_offset[partition_id + 1]) {
          ...
          csc_forward[tmp]
                     [(src - partition_offset[partition_id]) / CHUNK_LENGTH]
                         .numOfEdge++;
          l1++;
        }
        if (partition_offset[partition_id] < dst &&
            dst < partition_offset[partition_id + 1]) {
          for (tmp = 0; tmp < partitions; tmp++) {
          ...
          csc_backward[tmp]
                      [(dst - partition_offset[partition_id]) / CHUNK_LENGTH]
                          .numOfEdge++;
          l2++;
        }
        //__sync_fetch_and_add(&out_degree[src], 1);
    ...
    for (int i = 0; i < csc_partition_size[0]; i++) {
      for (int j = 0; j < csc_partition_size[1]; j++) {
        this->csc_forward[i][j].dstList =
            new VertexId[csc_forward[i][j].numOfEdge];
        this->csc_forward[i][j].srcList =
            new VertexId[csc_forward[i][j].numOfEdge];
        this->csc_backward[i][j].dstList =
            new VertexId[csc_backward[i][j].numOfEdge];
        this->csc_backward[i][j].srcList =
            new VertexId[csc_backward[i][j].numOfEdge];
      }
    }
    ...
    while (read_bytes <
      ...
      for (EdgeId e_i = 0; e_i < curr_read_edges; e_i++) {
        ...
        if (partition_offset[partition_id] < src &&
          ...
          csc_forward[tmp][(src - partition_offset[partition_id]) /
                           CHUNK_LENGTH]
              .srcList[csc_forward[tmp][(src - partition_offset[partition_id]) /
                                        CHUNK_LENGTH]
                           .counter] = src;
          csc_forward[tmp][(src - partition_offset[partition_id]) /
                           CHUNK_LENGTH]
              .dstList[csc_forward[tmp][(src - partition_offset[partition_id]) /
                                        CHUNK_LENGTH]
                           .counter] = dst;
          csc_forward[tmp]
                     [(src - partition_offset[partition_id]) / CHUNK_LENGTH]
        ...
        if (partition_offset[partition_id] < dst &&
          ...
          csc_backward[tmp][(dst - partition_offset[partition_id]) /
                            CHUNK_LENGTH]
              .srcList[csc_backward[tmp]
                                   [(dst - partition_offset[partition_id]) /
                                    CHUNK_LENGTH]
                                       .counter] = dst;
          csc_backward[tmp][(dst - partition_offset[partition_id]) /
                            CHUNK_LENGTH]
              .dstList[csc_backward[tmp]
                                   [(dst - partition_offset[partition_id]) /
                                    CHUNK_LENGTH]
                                       .counter] = src;
          csc_backward[tmp]
    ...
    for (int i = 0; i < csc_partition_size[0]; i++) {
      for (int j = 0; j < csc_partition_size[1]; j++) {
        printf("number of edge %d %d\n", this->csc_forward[i][j].numOfEdge,
               this->csc_forward[i][j].counter);
        alll += this->csc_forward[i][j].numOfEdge;
        //         this->csc_forward[i][j].dstList=new
        //         VertexId[csc_forward[i][j].numOfEdge];
        //         this->csc_forward[i][j].srcList=new
        //         VertexId[csc_forward[i][j].numOfEdge];
        //         this->csc_backward[i][j].dstList=new
        //         VertexId[csc_backward[i][j].numOfEdge];
        //         this->csc_backward[i][j].srcList=new
        //         VertexId[csc_backward[i][j].numOfEdge];
      }
    }
    printf("%d,%d\n", alll,
  ...
  void reorder_COO() {
    ...
    for (int i = 0; i < this->edges; i++) {
      ...
      // }
      _graph_cpu
          ->partition_offset[get_partition_id(_graph_cpu->src()[i]) + 1] += 1;
    }
    ...
    delete[] write_offset;
    std::cout << "re order " << partitions << " "
              << _graph_cpu->partition_offset[1] << std::endl;
  }
  ...
  void compress() {
    ...
    for (int s_i = 0; s_i < sockets; s_i++) {
      for (VertexId p_v_i = 0;
           p_v_i < this->compressed_incoming_adj_vertices[s_i]; p_v_i++) {
        VertexId v_i = this->compressed_incoming_adj_index[s_i][p_v_i].vertex;
        compress_flag[v_i] = true;
      }
    }
    for (VertexId v_i = partition_offset[partition_id];
         v_i < partition_offset[partition_id]; v_i++) {
      compress_flag[v_i] = true;
    }
    count = 0;
    for (VertexId v_i = 0; v_i < vertices; v_i++) {
