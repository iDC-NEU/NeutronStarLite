...
typedef struct factor{
    ...
    factor(){
        memset(data, 0,VECTOR_LENGTH*sizeof(float));
    }
...
int vectorMulMatrix(float  *dst, float * src, float**weight){//vector*weight  cachefriendly multi.
    for(int i=0;i<VECTOR_LENGTH;i++){
        dst[i]=0.0;
    }
    for(int j=0;j<VECTOR_LENGTH;j++){
...
int matrixMulVector(float *dst, float **weight, float * src){//vector*weight  cachefriendly multi.
    for(int i=0;i<VECTOR_LENGTH;i++){
        dst[i]=0;
    for(int j=0;j<VECTOR_LENGTH;j++){
