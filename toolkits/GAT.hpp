...
public:
  ...
  GAT_impl(Graph<Empty> *graph_, int iterations_, bool process_local = false,
  ...
  void init_graph() {
    ...
    gt->GenerateGraphSegment(subgraphs, GPU_T,
                             [&](VertexId src, VertexId dst) { return 0; });
  ...
  NtsVar edge_Backward(NtsVar &message_grad, NtsVar &src_grad,
                       NtsScheduler *nts) {
    I_data["msg_grad"] = at::_sparse_softmax_backward_data(
        nts->PrepareMessage(message_grad), I_data["atten"], nts->BYDST(),
        I_data["w"]);
  ...
  NtsVar edge_Forward(NtsVar &src_input, NtsVar &src_input_transfered,
    ...
    src_input_transfered = I_data["src_trans"] =
        P[layer * 2]->forward(I_data["src"]);
    I_data["dst_trans"] = dst_input_transfered; // finish apply W
    I_data["msg"] = torch::cat({nts->ScatterSrc(I_data["src_trans"]),
                                nts->ScatterDst(I_data["dst_trans"])},
                               1);
    I_data["msg"] = torch::leaky_relu(
        torch::exp(P[layer * 2 + 1]->forward(I_data["msg"])), 1.0);
  ...
  void Backward() {
    ...
    for (int i = graph->gnnctx->layer_size.size() - 2; i >= 0; i--) {
      ...
      gt->GraphPropagateBackwardEdgeComputation(
          X[i], grad_to_Y, X_grad[i], subgraphs,
          [&](NtsVar &master_input) { // pre computation
            return preComputation(master_input);
          },
          [&](NtsVar &mirror_input, NtsVar &mirror_input_transfered, NtsVar &X,
              NtsVar &X_trans, NtsScheduler *nts) { // edge computation
            return edge_Forward(mirror_input, mirror_input_transfered, X,
                                X_trans, nts);
          },
  ...
  void Forward() {
    ...
    for (int i = 0; i < graph->gnnctx->layer_size.size() - 1; i++) {
      graph->rtminfo->curr_layer = i;
      gt->GraphPropagateForwardEdgeComputation(
          X[i], Y[i], subgraphs,
          [&](NtsVar &master_input) { // pre computation
            return preComputation(master_input);
          },
