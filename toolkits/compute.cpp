...
void compute(Graph<Empty> *graph, int iterations) {
  ...
  for (int i_i = 0; i_i < iterations; i_i++) {
    ...
    inter->W.set_data(Gnn_v1->forward(
        pytool->x
            [0])); // torch::from_blob(embedding->Gnn_v1->forward(pytool->x[0]).accessor<float,2>().data(),{embedding->rownum,VECTOR_LENGTH});
    ...
    for (int i = 0; i < graph->owned_vertices; i++) {
      new_combine_grad =
          new_combine_grad +
          pytool->x[0][i]
                  .reshape({SIZE_LAYER_1, 1})
                  .mm(pytool->localGrad[0][i].reshape({1, SIZE_LAYER_2})) *
    ...
    if (i_i == (iterations - 1) && graph->partition_id == 0) {
      ...
      if (i_i == iterations - 1) {
        ...
        for (int k = 0; k < embedding->rownum; k++) {
          ...
          for (int i = 0; i < LABEL_NUMBER; i++) {
            if (max <
                tt.accessor<ValueType, 2>().data()[k * LABEL_NUMBER + i]) {
      ...
      for (int k = graph->vertices; k < NODE_NUMBER; k++) {
        ...
        torch::Tensor final_ =
            torch::relu(test.mm(Gnn_v1->W)).mm(Gnn_v2->W).log_softmax(1);
        for (int i = 0; i < LABEL_NUMBER; i++) {
