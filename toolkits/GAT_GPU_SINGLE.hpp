...
public:
  ...
  GAT_GPU_SINGLE_impl(Graph<Empty> *graph_, int iterations_,
  ...
  NtsVar edge_Forward(NtsVar &master_input) {
    ...
    E_in[0] =
        torch::cat({m_i_t, graph->Nts->ScatterDst(src_input_transfered)}, 1);
    E_out[0] =
        torch::leaky_relu(torch::exp(P[layer * 2 + 1]->forward(E_in[0])), 1.0);
    // E_out[0].backward(torch::ones_like(E_out[0]));
    // std::cout<<"hello "<<mirror_in[0].size(0)<<" "<<mirror_in[0].size(1)<<"
    // "<<mirror_in[0].grad().dim()<<std::endl;
  ...
  NtsVar edge_Backward(NtsVar &message_grad, NtsVar &src_input_trans,
    ...
    //     E_in[pos(graph->cpp,layer)]=torch::cat({nts->ScatterSrc(src_input_transfered),
    //     nts->ScatterDst(dst_input_transfered)},1);
    E_out[pos(graph->cpp, layer)] = torch::leaky_relu(
    ...
    E_out[pos(graph->cpp, layer)].backward(
        torch::ones_like(E_out[pos(graph->cpp, layer)]), true);
    //      I_data["src_trans"].backward(src_grad,true);
    // std::cout<<"hello "<<src_input_trans.size(0)<<"
    // "<<src_input_trans.size(1)<<" "<<src_input_trans.grad().dim()<<std::endl;
