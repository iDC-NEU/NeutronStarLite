...
public:
    ...
    GAT_GPU_impl(Graph<Empty> *graph_, int iterations_, bool process_local = false, bool process_overlap = false){
        graph=graph_;
...
//NtsVar edge_Forward(NtsVar &src_input_transfered, NtsVar &dst_input_transfered, 
//                                    NtsScheduler* nts){
//     size_t layer =graph->rtminfo->curr_layer;
//     I_data["msg"]=torch::cat({nts->ScatterSrc(src_input_transfered), nts->ScatterDst(dst_input_transfered)},1);                    
...
NtsVar edge_Forward(NtsVar &src_input_transfered, NtsVar &dst_input_transfered, 
     ...
     E_in[pos(graph->cpp,layer)]=torch::leaky_relu(torch::exp(P[layer*2+1]->forward(E_in[pos(graph->cpp,layer)])),1.0);  
...
NtsVar edge_Backward(NtsVar &message_grad, NtsVar &src_input_trans, 
    ...
    NtsVar msg_grad=at::_sparse_softmax_backward_data(
     ...
     //printf("end softmax\n");
      E_in[pos(graph->cpp,layer)].values().backward(msg_grad.coalesce().values(),true);
//      I_data["src_trans"].backward(src_grad,true);
      std::cout<<"hello "<<src_input_trans.size(0)<<" "<<src_input_trans.size(1);
...
void Test(long s){// 0 train, //1 eval //2 test
    ...
    NtsVar all_train=X[graph->gnnctx->layer_size.size()-1]
                    .argmax(1).to(torch::kLong)
                    .eq(L_GT_G).to(torch::kLong)
...
void Backward(){
    ...
    gt->PropagateBackwardEdgeGPU(Y[i], grad_to_X, Y_grad[i], subgraphs,
        [&](NtsVar &mirror_input_trans, NtsVar &X_trans, NtsScheduler* nts){//edge computation
...
//    }
    ...
    gt->PropagateForwardEdgeGPU(Y[i],X[i+1],subgraphs
                ,[&](NtsVar &mirror_input_trans, NtsVar &master_input_trans, NtsScheduler* nts){//edge computation
