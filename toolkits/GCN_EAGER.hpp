...
public:
    ...
    GCN_EAGER_impl(Graph<Empty> *graph_, int iterations_, bool process_local = false, bool process_overlap = false){
        graph=graph_;
...
void Backward(){
    ...
    gt->GraphPropagateBackward(grad_to_X, X_grad[i], subgraphs);
    vertexBackward();
    }
}
...
void Forward(){
    ...
    gt->GraphPropagateForward(Y[i], X[i+1], subgraphs);
    }
