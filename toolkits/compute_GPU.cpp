...
void compute_GPU(Graph<Empty> *graph, int iterations) {
  ...
  torch::Tensor inter1_gpu = torch::zeros(
      ...
      at::TensorOptions().device_index(0).requires_grad(true).dtype(
          torch::kFloat));
  torch::Tensor inter2_gpu = torch::zeros(
      {embedding->rownum, SIZE_LAYER_2},
      at::TensorOptions().device_index(0).requires_grad(true).dtype(
          torch::kFloat));
  Gnn_v2->to(GPU);
  ...
  for (int i_i = 0; i_i < iterations; i_i++) {
    ...
    inter1_gpu.set_data(Gnn_v1->forward(
        pytool->x[0]
            .cuda())); // torch::from_blob(embedding->Gnn_v1->forward(pytool->x[0]).accessor<float,2>().data(),{embedding->rownum,VECTOR_LENGTH});
    ...
    // std::cout<<inter1->W.grad().size(0)<<"
    // "<<inter1->W.grad().size(1)<<std::endl;
    ...
    ag_e->redirect_input_output(
        grad0_gpu.packed_accessor<float, 2>().data(),
    ...
    if (i_i == (iterations - 1) && graph->partition_id == 0) {
      ...
      if (i_i == iterations - 1) {
        ...
        std::cout << "\ncorrect number on training:" << correct << "\t"
                  << ((ValueType)correct / (ValueType)graph->vertices)
                  << std::endl;
      }
