...
void compute_dist_GPU(Graph<Empty> *graph, int iterations) {
  ...
  ag_e->reconfig_data(graph->owned_vertices, SIZE_LAYER_2,
                      graph->owned_vertices, SIZE_LAYER_1, TENSOR_TYPE);
  ag_e->init_intermediate_gradient();

  graph_engine *gr_e = new graph_engine();
  // graph->generate_COO(active);
  // std::cout<<graph->partition_offset[0]<<" "<<graph->partition_offset[1]<<"
  // "<<graph->partition_offset[2]<<std::endl; graph->reorder_COO();
  ...
  for (VertexId vtx = 0; vtx < graph->vertices; vtx++) {
    ...
    if (graph->incoming_adj_index_backward[0][vtx + 1] == 0)
      graph->incoming_adj_index_backward[0][vtx + 1] =
          graph->incoming_adj_index_backward[0][vtx];
  }
  ...
  std::cout << "something error" << incoming_adj_index[graph->vertices - 1]
            << " "
            << (VertexId)graph->incoming_adj_index[0][graph->vertices - 1]
            << std::endl;

  gr_e->load_graph(graph->vertices, edges_for, false, graph->vertices,
                   edges_back, false, incoming_adj_index,
                   (VertexId *)graph->incoming_adj_list[0],
                   incoming_adj_index_backward,
                   (VertexId *)graph->incoming_adj_list_backward[0],
                   graph->partition_offset[graph->partition_id],
                   graph->partition_offset[graph->partition_id + 1], 0,
                   graph->vertices, SIZE_LAYER_1);
  /*1 INIT STAGE*/
  GnnUnit *Gnn_v1 = new GnnUnit(SIZE_LAYER_1, SIZE_LAYER_2);
  GnnUnit *Gnn_v2 = new GnnUnit(SIZE_LAYER_2, OUTPUT_LAYER_3);
  std::cout << Gnn_v1->W.size(0) << "   " << Gnn_v1->W.size(1) << " "
            << graph->partition_id << std::endl;
  ...
  torch::Tensor W_for_gpu =
      torch::from_blob(weight, {edges_for, 1}, torch::kFloat).cuda();
  torch::Tensor W_back_gpu =
      torch::from_blob(weight_backward, {edges_back + 1, 1}, torch::kFloat)
          .cuda();
  torch::Tensor Out0_gpu;
  torch::Tensor Out1_gpu;
  torch::Tensor Y0_gpu_buffered = torch::zeros(
      {graph->vertices, SIZE_LAYER_1},
      at::TensorOptions().device_index(0).requires_grad(true).dtype(
          torch::kFloat));
  torch::Tensor Y1_gpu_buffered = torch::zeros(
      {graph->vertices, SIZE_LAYER_2},
      at::TensorOptions().device_index(0).requires_grad(true).dtype(
          torch::kFloat));
  torch::Tensor Y1_inv_gpu_buffered = torch::zeros(
      {graph->vertices, SIZE_LAYER_1},
      at::TensorOptions().device_index(0).requires_grad(true).dtype(
          torch::kFloat));
  ...
  for (int i_i = 0; i_i < iterations; i_i++) {
    ...
    inter1_gpu.set_data(Gnn_v1->forward(
        Y0_gpu)); // torch::from_blob(embedding->Gnn_v1->forward(pytool->x[0]).accessor<float,2>().data(),{embedding->rownum,VECTOR_LENGTH});
    ...
    // inv layer 2;
    torch::Tensor aggregate_grad2 =
        unified_parameter<ValueType>(comm1, Gnn_v2->W.grad().cpu());
    Gnn_v2->learn_gpu(aggregate_grad2.cuda(), learn_rate); // reset from new

    // inv layer 1;
    // 2->1
    gr_e->backward_one_step(
        Y1_gpu.grad().packed_accessor<float, 2>().data(),
        Y1_inv_gpu_buffered.packed_accessor<float, 2>().data(),
        W_back_gpu.packed_accessor<float, 2>().data(), SCALA_TYPE,
        SIZE_LAYER_2);
    ...
    torch::Tensor aggregate_grad =
        unified_parameter<ValueType>(comm, new_combine_grad.cpu());
...
void compute_dist_gf_GPU(Graph<Empty> *graph, int iterations) {
  ...
  //     std::cout<<graph->_graph_cpu->srcList[i]<<"
  //     "<<graph->_graph_cpu->dstList[i]<<std::endl;
  // }
  std::cout << graph->owned_vertices << " " << graph->edges << std::endl;
  ...
  torch::Tensor Y1_inv_gpu_buffered = torch::zeros(
      ...
      at::TensorOptions().device_index(0).requires_grad(true).dtype(
          torch::kFloat));
  torch::Tensor Y0_inv_gpu_buffered = torch::zeros(
      {graph->vertices, SIZE_LAYER_2},
      at::TensorOptions().device_index(0).requires_grad(true).dtype(
          torch::kFloat));
  ...
  for (int i_i = 0; i_i < iterations; i_i++) {
    ...
    gr_e->backward_one_step(
        tmp.packed_accessor<float, 2>().data(),
...
/*GPU dist*/ void compute_dist_GPU_with_new_system(Graph<Empty> *graph,
                                                   int iterations) {
  if (graph->partition_id == 0)
    printf("dist GCN");
  ValueType learn_rate = 0.01;
  ...
  for (int i_s; i_s < graph->sockets; i_s++) {
    for (VertexId vtx = 0; vtx < graph->vertices; vtx++) {
      if (graph->incoming_adj_index[i_s][vtx + 1] == 0)
        graph->incoming_adj_index[i_s][vtx + 1] =
            graph->incoming_adj_index[i_s][vtx];
      if (graph->incoming_adj_index_backward[i_s][vtx + 1] == 0)
        graph->incoming_adj_index_backward[i_s][vtx + 1] =
            graph->incoming_adj_index_backward[i_s][vtx];
    }
  }
  ...
  graph->process_vertices<ValueType>( // init  the vertex state.
      [&](VertexId vtx) {
        for (int i = 0; i < SIZE_LAYER_1; i++) {
          embedding->initStartWith(
              vtx, embedding->con[vtx].att[i],
              i); // embedding->con[vtx].att[i],i);//embedding->con[vtx].att[i]
        }
        return (ValueType)1;
      },
      active);
  ...
  torch::Tensor Y0_gpu = torch::zeros(
      ...
      at::TensorOptions().device_index(0).requires_grad(true).dtype(
          torch::kFloat));
  torch::Tensor Y0_cpu =
      torch::zeros({embedding->rownum, SIZE_LAYER_1}, torch::kFloat);
  torch::Tensor Y1_gpu = torch::zeros(
      {embedding->rownum, SIZE_LAYER_2},
      at::TensorOptions().device_index(0).requires_grad(true).dtype(
          torch::kFloat));
  torch::Tensor Y1_cpu =
      torch::zeros({embedding->rownum, SIZE_LAYER_2}, torch::kFloat);
  torch::Tensor Y1_inv_gpu = torch::zeros(
      {embedding->rownum, SIZE_LAYER_2},
      at::TensorOptions().device_index(0).requires_grad(true).dtype(
          torch::kFloat));
  ...
  torch::Tensor Out0_gpu;
  torch::Tensor Out1_gpu;
  torch::Tensor Y0_cpu_buffered =
      torch::zeros({graph->vertices, SIZE_LAYER_1}, torch::kFloat);

  torch::Tensor Y1_cpu_buffered =
      torch::zeros({graph->vertices, SIZE_LAYER_2}, torch::kFloat);
  torch::Tensor Y1_inv_cpu_buffered =
      torch::zeros({graph->vertices, SIZE_LAYER_2}, torch::kFloat);
  ...
  for (int i_i = 0; i_i < iterations; i_i++) {
    ...
    int testid = 0;
    // std::cout<<"wrong with "<<graph->in_degree[testid]<<
    // graph->partition_id<<std::endl;
    graph_time = 0;
    graph_time -= get_time();
    propagate_forward_gpu_shard(graph, X0_cpu, Y0_cpu_buffered, edge_list,
                                SIZE_LAYER_1);
    ...
    graph_time = 0;
    graph_time -= get_time();
    propagate_forward_gpu_shard(graph, Out0_gpu.cpu(), Y1_cpu_buffered,
    ...
    compute_time = 0;
    compute_time -= get_time();
    Gnn_v2->learn_gpu(aggregate_grad2.cuda(),
                      learn_rate); // reset from new
                                   //        CUDA_SYNCHRONIZE();
    compute_time += get_time();
    all_compute_time += compute_time;
    // inv layer 1;
    ...
    graph_time = 0;
    graph_time -= get_time();
    propagate_forward_gpu_shard(graph, Y1_gpu.grad().cpu(), Y1_inv_cpu_buffered,
                                edge_list, SIZE_LAYER_2);
    graph_time += get_time();
    all_graph_time += graph_time;
    copy_time = 0;
...
/*GPU dist*/ void compute_dist_GPU_with_new_system_CSC(Graph<Empty> *graph,
                                                       int iterations,
                                                       bool GPUaggregator) {
  if (graph->partition_id == 0)
    printf("dist GCN CSC\n");
  ...
  torch::Tensor X0_cpu =
      torch::from_blob(embedding->start_v + embedding->start,
                       {embedding->rownum, SIZE_LAYER_1}, torch::kFloat);
  torch::Tensor Y0_gpu =
      torch::zeros({embedding->rownum, SIZE_LAYER_1},
  ...
  for (int i_i = 0; i_i < iterations; i_i++) {
    ...
    if (i_i == 0 || true) {
      ...
      graph_time = 0;
      graph_time -= get_time();
      propagate_forward_gpu_shard_CSC(graph, X0_cpu, Y0_cpu_buffered,
                                      csc_segment, SIZE_LAYER_1);
      // std::cout<<"validation with"<<Y0_cpu_buffered[testid][0]<<std::endl;
      graph_time += get_time();
      all_graph_time += graph_time;
    }
    ...
    graph_time = 0;
    graph_time -= get_time();
    propagate_forward_gpu_shard_CSC(graph, Out0_gpu.cpu(), Y1_cpu_buffered,
                                    csc_segment, SIZE_LAYER_2);
    graph_time += get_time();
    all_graph_time += graph_time;
    ...
    graph_time = 0;
    graph_time -= get_time();
    propagate_forward_gpu_shard_CSC(graph, Y1_gpu.grad().cpu(),
                                    Y1_inv_cpu_buffered, csc_segment,
                                    SIZE_LAYER_2);
    graph_time += get_time();
    all_graph_time += graph_time;
...
/*GPU dist*/ void
compute_dist_GPU_with_CSC_overlap_1(Graph<Empty> *graph, int iterations,
  ...
  GnnUnit *Gnn_v1 =
      new GnnUnit(graph->gnnctx->layer_size[0], graph->gnnctx->layer_size[1]);
  GnnUnit *Gnn_v3 =
      new GnnUnit(graph->gnnctx->layer_size[0], graph->gnnctx->layer_size[1]);
  GnnUnit *Gnn_v2 =
      new GnnUnit(graph->gnnctx->layer_size[1], graph->gnnctx->layer_size[2]);
  ...
  GTensor<ValueType, long, MAX_LAYER> *gt =
      new GTensor<ValueType, long, MAX_LAYER>(graph, active);

  // intermediate tensors
  torch::Tensor new_combine_grad =
      torch::zeros({graph->gnnctx->layer_size[0], graph->gnnctx->layer_size[1]},
                   torch::kFloat)
          .cuda();

  torch::Tensor inter1_gpu = torch::zeros(
      {graph->gnnctx->l_v_num, graph->gnnctx->layer_size[1]},
      at::TensorOptions().device_index(0).requires_grad(true).dtype(
          torch::kFloat));
  torch::Tensor X0_cpu =
      torch::ones({graph->gnnctx->l_v_num, graph->gnnctx->layer_size[0]},
  ...
  torch::Tensor Out1_gpu;
  torch::Tensor loss;
  float *Y0_cpu_buffered = (float *)cudaMallocPinned(
      ((long)graph->vertices) * graph->gnnctx->max_layer * sizeof(float));
...
/*GPU dist*/ void
compute_dist_GPU_with_CSC_overlap(Graph<Empty> *graph, int iterations,
                                  bool process_local = false,
                                  bool process_overlap = false) {
  ...
  torch::Tensor X0_gpu = X0_cpu.cuda();
  torch::Tensor Y0_gpu =
      torch::zeros({graph->gnnctx->l_v_num, graph->gnnctx->layer_size[0]},
                   at::TensorOptions().device_index(0).dtype(torch::kFloat));
  torch::Tensor Y1_gpu = torch::zeros(
      {graph->gnnctx->l_v_num, graph->gnnctx->layer_size[1]},
      at::TensorOptions().device_index(0).requires_grad(true).dtype(
  ...
  for (int i_i = 0; i_i < iterations; i_i++) {
    ...
    gt->Process_GPU_overlap_explict(X0_gpu, Y0_cpu_buffered, Y0_gpu,
                                    csc_segment);
    ...
    int test_id = 0;
    std::cout << graph->in_degree_for_backward
                     [test_id + graph->partition_offset[graph->partition_id]]
              << " " << Y0_gpu[test_id][0] << std::endl;
    ...
    gt->Process_GPU_overlap_explict(Out0_gpu, Y0_cpu_buffered, Y1_gpu,
                                    csc_segment);
    ...
    graph->rtminfo->curr_layer = 1;
    gt->Process_GPU_overlap_explict(Y1_gpu.grad(), Y0_cpu_buffered, Y1_inv_gpu,
                                    csc_segment);
    ...
    if (i_i == (iterations - 1)) { //&&graph->partition_id==0
      ...
      MPI_Allreduce(&exec_time, &another_time, 1, l_vid_t, MPI_SUM,
                    MPI_COMM_WORLD);
      MPI_Allreduce(&graph->all_replication_time, &mean_time, 1, l_vid_t,
...
/*GPU dist GF*/ void
compute_dist_GPU_with_CSC_overlap_exchange(Graph<Empty> *graph,
                                           int iterations) {
  ...
  embedding->init(graph);
  embedding->readlabel_(graph);
  Network<ValueType> *comm = new Network<ValueType>(graph, MAX_LAYER,
  MAX_LAYER); Network<ValueType> *comm1 = new Network<ValueType>(graph,
  MAX_LAYER, MAX_LAYER); comm->setWsize(MAX_LAYER, MAX_LAYER);
  comm1->setWsize(MAX_LAYER, MAX_LAYER);
  tensorSet *pytool = new tensorSet(2);
  ...
  for (int i_s; i_s < graph->sockets; i_s++)
  {
      for (VertexId vtx = 0; vtx < graph->vertices; vtx++)
      {
          if (graph->incoming_adj_index[i_s][vtx + 1] == 0)
              graph->incoming_adj_index[i_s][vtx + 1] =
  graph->incoming_adj_index[i_s][vtx]; if
  (graph->incoming_adj_index_backward[i_s][vtx + 1] == 0)
              graph->incoming_adj_index_backward[i_s][vtx + 1] =
  graph->incoming_adj_index_backward[i_s][vtx];
      }
  }
  ...
  pytool->registOptimizer(torch::optim::SGD(Gnn_v1->parameters(), 0.05)); //new
  pytool->registOptimizer(torch::optim::SGD(Gnn_v2->parameters(), 0.05)); //new
  pytool->registLabel<long>(embedding->label,
  graph->partition_offset[graph->partition_id],
                            graph->partition_offset[graph->partition_id + 1] -
  graph->partition_offset[graph->partition_id]); //new
  ...
  layer_size.push_back(SIZE_LAYER_2);
  layer_size.push_back(OUTPUT_LAYER_3);
  GTensor<ValueType, long, MAX_LAYER> *gt = new GTensor<ValueType, long,
  MAX_LAYER>(graph, embedding, active, 2, layer_size);

  torch::Tensor new_combine_grad = torch::zeros({SIZE_LAYER_1, SIZE_LAYER_2},
  torch::kFloat).cuda(); graph->process_vertices<ValueType>( //init  the vertex
  state.
      [&](VertexId vtx) {
          for (int i = 0; i < SIZE_LAYER_1; i++)
          {
              embedding->initStartWith(vtx, embedding->con[vtx].att[i], i);
  //embedding->con[vtx].att[i],i);//embedding->con[vtx].att[i]
          }
          return (ValueType)1;
      },
      active);
  ...
  torch::Device GPU(torch::kCUDA, 0);
  torch::Tensor target_gpu = pytool->target.cuda();
  torch::Tensor inter1_gpu = torch::zeros({embedding->rownum, SIZE_LAYER_2},
  at::TensorOptions().device_index(0).requires_grad(true).dtype(torch::kFloat));
  Gnn_v2->to(GPU);
  Gnn_v1->to(GPU);
  torch::Tensor X0_gpu_trans = torch::zeros({embedding->rownum, SIZE_LAYER_2},
  torch::kFloat).cuda(); torch::Tensor X0_cpu =
  torch::from_blob(embedding->start_v + embedding->start, {embedding->rownum,
  SIZE_LAYER_1}, torch::kFloat); torch::Tensor X0_gpu = X0_cpu.cuda();
  torch::Tensor Y0_gpu = torch::zeros({embedding->rownum, SIZE_LAYER_2},
  at::TensorOptions().device_index(0).requires_grad(true).dtype(torch::kFloat));
  //torch::Tensor
  Y0_cpu=torch::zeros({embedding->rownum,SIZE_LAYER_1},torch::kFloat);
  torch::Tensor Y1_gpu = torch::zeros({embedding->rownum, SIZE_LAYER_2},
  at::TensorOptions().device_index(0).requires_grad(true).dtype(torch::kFloat));
  //torch::Tensor
  Y1_cpu=torch::zeros({embedding->rownum,SIZE_LAYER_2},torch::kFloat);
  torch::Tensor Y1_inv_gpu = torch::zeros({embedding->rownum, SIZE_LAYER_2},
  at::TensorOptions().device_index(0).requires_grad(true).dtype(torch::kFloat));
  //torch::Tensor Y1_inv_cpu=torch::zeros({embedding->rownum,SIZE_LAYER_2},
  torch::kFloat); torch::Tensor Y0_inv_gpu = torch::zeros({embedding->rownum,
  SIZE_LAYER_2},
  at::TensorOptions().device_index(0).requires_grad(true).dtype(torch::kFloat));
  torch::Tensor Out0_gpu;

  torch::Tensor Out1_gpu;
  float *Y0_cpu_buffered = (float *)cudaMallocPinned(((long)graph->vertices) *
  SIZE_LAYER_1 * sizeof(float));
  ...
  epoch+++++++++++++++++++++++++++++++++++++++++++++" << std::endl;
      ...
      gt->Process_GPU_overlap<SIZE_LAYER_2>(X0_gpu_trans, Y0_cpu_buffered,
  Y0_gpu, csc_segment, true);
      ...
      gt->Process_GPU_overlap<SIZE_LAYER_2>(Out0_gpu, Y0_cpu_buffered, Y1_gpu,
  csc_segment, false);
      //  gt->Sync_data_gpu<SIZE_LAYER_2>(Y1_cpu_buffered,Y1_gpu);
      graph_time += get_time();
      all_graph_time += graph_time;
      ...
      torch::Tensor aggregate_grad2 = unified_parameter<ValueType>(comm1,
  Gnn_v2->W.grad().cpu());
      ...
      gt->Process_GPU_overlap<SIZE_LAYER_2>(Y1_gpu.grad(), Y0_cpu_buffered,
  Y1_inv_gpu, csc_segment, false);
      //gt->Sync_data_gpu<SIZE_LAYER_2>(Y1_inv_cpu_buffered,Y1_inv_gpu);
      graph_time += get_time();
      all_graph_time += graph_time;
      ...
      gt->Process_GPU_overlap<SIZE_LAYER_2>(tmp, Y0_cpu_buffered, Y0_inv_gpu,
  csc_segment, false);
      ...
      torch::Tensor aggregate_grad = unified_parameter<ValueType>(comm,
  new_combine_grad.cpu());
      ...
      { //&&graph->partition_id==0
          ...
          if (graph->partition_id == 0)
          {
              ...
              printf("graph_time=%lf(s)\n", all_graph_time);
              printf("communicate_extract+send=%lf(s)\n",
  graph->all_compute_time); printf("communicate_processing_received=%lf(s)\n",
  graph->all_overlap_time);
              printf("communicate_processing_received.copy=%lf(s)\n",
  graph->all_recv_copy_time);
              printf("communicate_processing_received.kernel=%lf(s)\n",
  graph->all_recv_kernel_time);
              printf("communicate_processing_received.wait=%lf(s)\n",
  graph->all_recv_wait_time); printf("communicate_wait=%lf(s)\n",
  graph->all_wait_time); printf("streamed kernel_time=%lf(s)\n",
  graph->all_kernel_time); printf("streamed movein_time=%lf(s)\n",
  graph->all_movein_time); printf("streamed moveout_time=%lf(s)\n",
  graph->all_moveout_time); printf("graph repliation time=%lf(s)\n",
  graph->all_replication_time); printf("DEBUG INFO
  END:::::::::::::::::::::::::::::::\n");
          }
