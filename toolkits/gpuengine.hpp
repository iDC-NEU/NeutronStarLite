...
/*GPU single order*/ void compute_single_GPU_old(Graph<Empty> *graph, int iterations)
{
    ValueType learn_rate = 0.01;
    VertexSubset *active = graph->alloc_vertex_subset();
    active->fill();
    ...
    for (int i_i = 0; i_i < iterations; i_i++)
    {
        ...
        nn_time -= get_time();
        printf("wrong\n");
        std::cout << inter1_gpu.size(0) << " " << inter1_gpu.size(1) << "\t\t\t\t" << Y0_gpu.size(0) << " " << Y0_gpu.size(1) << std::endl;
