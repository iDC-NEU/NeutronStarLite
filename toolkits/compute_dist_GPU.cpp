...
void compute_dist_GPU(Graph<Empty> *graph, int iterations) {
  ...
  Network<ValueType> *comm =
      new Network<ValueType>(graph, WEIGHT_ROW, WEIGHT_COL);
  ...
  GTensor<ValueType, long, VECTOR_LENGTH> *gt =
      new GTensor<ValueType, long, VECTOR_LENGTH>(graph, embedding, active, 2,
                                                  layer_size);
  ...
  for (int i_i = 0; i_i < iterations; i_i++) {
    ...
    // std::cout<<"validation
    // middle"<<comm_data.accessor<ValueType,2>().data()[(tes)*SIZE_LAYER_1]<<"
    // "<<graph->in_degree[tes+graph->partition_offset[graph->partition_id]]<<std::endl;
    ...
    // std::cout<<"validation
    // end"<<reads.accessor<ValueType,2>().data()[(tes-graph->partition_offset[graph->partition_id])*SIZE_LAYER_1]<<"
    // "<<graph->in_degree[tes]<<std::endl;
    ...
    // std::cout<<"valdate middle"<<s1.accessor<ValueType,2>().data()[0]<<"
    // "<<s2.accessor<ValueType,2>().data()[0]
    // <<" "<<aggregate_grad2.accessor<ValueType,2>().data()[0]<<"
    // "<<s3.accessor<ValueType,2>().data()[0]<<std::endl;
    ...
    //     // std::cout<<"valdate
    //     before"<<s1.accessor<ValueType,2>().data()[0]<<"
    //     "<<s2.accessor<ValueType,2>().data()[0]<<std::endl;
    // std::cout<<"valdate middle"<<s1.accessor<ValueType,2>().data()[0]<<"
    // "<<s2.accessor<ValueType,2>().data()[0]
    //     <<" "<<aggregate_grad2.accessor<ValueType,2>().data()[0]<<"
    //     "<<s3.accessor<ValueType,2>().data()[0]<<std::endl;
    ...
    if (i_i == (iterations - 1)) { //&&graph->partition_id==0
      ...
      if (i_i == iterations - 1) {
        //      std::cout<<pytool->y[1].softmax(1).log().accessor<float,2>().size(0)<<"
        //      "<<pytool->y[1].softmax(1).log().accessor<float,2>().size(1)<<std::endl;
        int correct = 0;
        for (int k = 0; k < embedding->rownum; k++) {
          ...
          for (int i = 0; i < LABEL_NUMBER; i++) {
            if (max <
                tt_cpu.accessor<ValueType, 2>().data()[k * LABEL_NUMBER + i]) {
              max =
                  tt_cpu.accessor<ValueType, 2>().data()[k * LABEL_NUMBER + i];
              id = i;
            }
        ...
        std::cout << "\ncorrect number on training:" << correct << "\t"
                  << ((ValueType)correct / (ValueType)graph->owned_vertices)
                  << "on id" << graph->partition_id << std::endl;
      }
      std::cout << "loss at" << graph->partition_id << "is :" << pytool->loss
                << std::endl;
      int correct_test = 0;
      for (int k = graph->vertices; k < NODE_NUMBER; k++) {
        ...
        long id = -1;
        torch::Tensor test =
            torch::from_blob(&(embedding->con[k].att[0]), {1, SIZE_LAYER_1});
        torch::Tensor final_ = torch::relu(test.mm(Gnn_v1->W.cpu()))
                                   .mm(Gnn_v2->W.cpu())
                                   .log_softmax(1);
        for (int i = 0; i < LABEL_NUMBER; i++) {
      ...
      std::cout << "valdate" << ses.accessor<ValueType, 2>().data()[0] << " "
                << ses1.accessor<ValueType, 2>().data()[0]
                << "correct number on testing:" << correct_test << "\t"
                << ((ValueType)correct_test /
                    (ValueType)(NODE_NUMBER - graph->vertices))
                << "on id" << graph->partition_id << std::endl;
    }
  }
  //     delete active;
