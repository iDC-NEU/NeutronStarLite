...
template<int CURRENT_LAYER_SIZE> 
    ...
    graph_->process_edges<int, nodeVector<CURRENT_LAYER_SIZE>>(// For EACH Vertex Processing
        ...
        printf("finish post_process\n");
        value_local[local_layer]=torch::from_blob(value_buffer[local_layer],{rownum_,CURRENT_LAYER_SIZE},torch::kFloat);
        printf("finish post_post_process\n");
        std::cout<<value_local[local_layer].sizes()<<std::endl;
        current_layer=local_layer;
        if(graph_->partition_id==1)
        {int tag=0;
            for(int i=0;i<rownum_;i++){
                if(value_buffer[local_layer][i*CURRENT_LAYER_SIZE]-(float)graph_->in_degree_for_backward[i+start_]<=0.00001)
...
template<int CURRENT_LAYER_SIZE>   
    void Propagate_backward(int local_layer){
      ...
      graph_->process_vertices<float>(//init  the vertex state.
        ...
        //start graph engine.
    //          std::cout<<"finish backward init"<<std::endl;  
   ...
   //       std::cout<<"finish backward propagate"<<std::endl;  
 ...
 std::cout<<"is here2"<<std::endl;
   //    gt->Test_Propagate<SIZE_LAYER_1>(0);
    ...
    for (int i_i = 0; i_i < iterations; i_i++) {
        ...
        // std::cout<<Gnn_v2->W.grad()<<std::endl;//graph->partition_offset[graph->partition_id]);
         
        torch::Tensor aggregate_grad2= unified_parameter<float>(comm1,Gnn_v2->W.grad().cpu());
       // std::cout<<aggregate_grad2<<std::endl;
         Gnn_v2->learn_gpu(aggregate_grad2.cuda(),learn_rate);//reset from new        
        std::cout<<inter2_gpu.grad().cpu().sizes()<<std::endl;
    ...
    graph->process_vertices<float>(//init  the vertex state.
        [&](VertexId vtx){
           ...
           partial_new_combine_grad[thread_id]=partial_new_combine_grad[thread_id]+
            x0_gpu[vtx-gt->start_].reshape({SIZE_LAYER_1,1}).mm(
            pytool->localGrad[0][vtx-gt->start_].reshape({1,SIZE_LAYER_2})
            ) *grad0_gpu[vtx-gt->start_].reshape({1,SIZE_LAYER_2});
