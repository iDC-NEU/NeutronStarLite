...
inline bool check(int v_id, Graph<Empty> *graph, VertexId *column_offset, VertexId *row_indices, Bitmap *cached, int degree_full)
{
    int count = 0;
    int error = 0;
    // std::cout<<"test";
    ...
    {
        if (cached->get_bit(row_indices[i]) || graph->get_partition_id(row_indices[i]) == graph->partition_id)
            count++;
        else
            error++;
...
inline void valid(int v_id, Graph<Empty> *graph, VertexId *column_offset, VertexId *row_indices, Bitmap *cached, int degree_full)
{ //check
    int count = 0;
    int error = 0;
    //std::cout<<"test";
...
void compute_active(Graph<Empty> *graph, int iterations, std::string path)
{ //分区算法
    ...
    memset(cahce_vertices_at_layer0_on_worker, 0, sizeof(int) * graph->partitions);
    ...
    memset(cahce_inter_vertices_on_worker_layer2, 0, sizeof(int) * graph->partitions);
    ...
    graph->process_edges<int, VertexId>(                       // For EACH Vertex Processing
        [&](VertexId dst, VertexAdjList<Empty> incoming_adj) { //pull
            ...
            if (cached_0_feature->get_bit(dst))
            {
                ...
                if (check(dst, graph, column_offset, row_indices, cached_0_feature, graph->in_degree_for_backward[dst]))
                {
                    cached_inter_vertices_when_layer2->set_bit(dst);
                    valid(dst, graph, column_offset, row_indices, cached_0_feature_2, graph->in_degree_for_backward[dst]);
                }
            }
        },
    ...
    for (int i = 0; i < graph->partitions; i++)
    {
        ...
        {
            for (int j = graph->partition_offset[i]; j < graph->partition_offset[i + 1]; j++)
            {
                ...
                if (cached_inter_vertices_when_layer2->get_bit(j))
                {
                    ...
                    cahce_edges_at_layer2_on_worker[graph->partition_id] += (graph->in_degree_for_backward[j] - indegree[j]);
                }
            }
        }
    }
    for (int i = 0; i < graph->partitions; i++)
    {
        reduce_comm_at_layer0_on_worker[graph->partition_id] += cahce_vertices_on_worker[i];
    }
    ...
    MPI_Allreduce(MPI_IN_PLACE, cahce_vertices_on_worker, graph->partitions, vid_t, MPI_SUM, MPI_COMM_WORLD);
    MPI_Allreduce(MPI_IN_PLACE, cahce_edges_at_layer0_on_worker, graph->partitions, vid_t, MPI_SUM, MPI_COMM_WORLD);
    MPI_Allreduce(MPI_IN_PLACE, reduce_comm_at_layer0_on_worker, graph->partitions, vid_t, MPI_SUM, MPI_COMM_WORLD);
    if (graph->partition_id == 0)
    {

        printf("Vertices: %d Edges: %d\n", graph->vertices, graph->edges);
        printf("DEBUG INFO AT THE FIRST LAYER######################################:\n");
        for (int i = 0; i < graph->partitions; i++)
        {
            printf("CACHED VERTICES %d, CACHED edges: %d,RECUDE comm:%d on worker %d\n",
                   cahce_vertices_on_worker[i], cahce_edges_at_layer0_on_worker[i], reduce_comm_at_layer0_on_worker[i], i);
        }
    }
    for (int i = 0; i < graph->partitions; i++)
    {
    ...
    if (graph->partition_id == 0)
    {
        printf("%d,%d overall message reduction: %f\n", allreduce_comm_at_layer0_on_worker,
               (graph->partitions * graph->vertices), ((float)allreduce_comm_at_layer0_on_worker / (graph->partitions * graph->vertices)));
        printf("###################################################################\n\n");
    }

    MPI_Allreduce(MPI_IN_PLACE, cahce_inter_vertices_on_worker_layer2, graph->partitions, vid_t, MPI_SUM, MPI_COMM_WORLD);
    MPI_Allreduce(MPI_IN_PLACE, cahce_edges_at_layer2_on_worker, graph->partitions, vid_t, MPI_SUM, MPI_COMM_WORLD);
    MPI_Allreduce(MPI_IN_PLACE, redundant_feature_cached, graph->partitions, vid_t, MPI_SUM, MPI_COMM_WORLD);
    MPI_Allreduce(MPI_IN_PLACE, redundant_feature_cached_layer2, graph->partitions, vid_t, MPI_SUM, MPI_COMM_WORLD);
    MPI_Allreduce(MPI_IN_PLACE, redundant_feature_cached_layerall, graph->partitions, vid_t, MPI_SUM, MPI_COMM_WORLD);
    for (int i = 0; i < graph->partitions; i++)
    {
    ...
    if (graph->partition_id == 0)
    {
        printf("\nDEBUG INFO AT THE SECOND LAYER######################################:\n");
        for (int i = 0; i < graph->partitions; i++)
        {
            printf("feature need to CACHED %d, CACHED edges layer2: %d,inter_layer RECUDE comm:%d on worker %d\n",
                   redundant_feature_cached[i], cahce_inter_vertices_on_worker_layer2[i], cahce_edges_at_layer2_on_worker[i], i);
            allreduce_comm_inter_for_layer2 += cahce_inter_vertices_on_worker_layer2[i];
        }
        printf("###################################################################\n\n");
        if (graph->partition_id == 0)
        {
            printf("%d,%d overall message reduction: %f\n", allreduce_comm_inter_for_layer2,
                   (graph->partitions * graph->vertices), ((float)allreduce_comm_inter_for_layer2 / (graph->partitions * graph->vertices)));
...
//inline int count_edge(int v_id,Graph<Empty> * graph, VertexId *column_offset,VertexId *row_indices,int degree_full,int error=0){//check
//    int count=0;
//    int error=0;
//    //std::cout<<"test";
//    for(int i=column_offset[v_id];i<column_offset[v_id+1];i++){
...
//    }
//}
void load_replicate(Graph<Empty> *graph, int iterations, std::string path)
{ //新的分区算法
...
void compute(Graph<Empty> *graph, int iterations)
{
    ...
    for (int i_i = 0; i_i < iterations; i_i++)
    {
        ...
        nn_time -= get_time();
        inter->W.set_data(Gnn_v1->forward(pytool->x[0])); //torch::from_blob(embedding->Gnn_v1->forward(pytool->x[0]).accessor<float,2>().data(),{embedding->rownum,MAX_LAYER});
...
for(int i=0;i<new_combine_grad_local.size();i++){
      new_combine_grad+=new_combine_grad_local[i];
        ...
        new_combine_grad = pytool->x[0].slice(0, 0, +graph->owned_vertices, 1).t().mm(pytool->localGrad[0] * gt->grad_local[0]);
...
/*CPU single order*/ void compute_gf(Graph<Empty> *graph, int iterations)
{
    ...
    for (int i_i = 0; i_i < iterations; i_i++)
    {
        ...
        inter->W.set_data(pytool->x[0]);      //torch::from_blob(embedding->Gnn_v1->forward(pytool->x[0]).accessor<float,2>().data(),{embedding->rownum,MAX_LAYER});
        pytool->y[0] = torch::relu(inter->W); //new
