...
public:
   void init(VertexId* partition_offset_,VertexId owned_vertices_,
    ...
    void init_message_buffer_master_to_mirror(){
        for (int i = 0; i < partitions; i++){
            for (int s_i = 0; s_i < sockets; s_i++){
                recv_buffer[i][s_i]->resize(size_of_msg(feature_size) * (partition_offset[i + 1] - partition_offset[i]) * sockets);
    ...
    void init_message_buffer_mirror_to_master(){
        ...
        for (int i = 0; i < partitions; i++){
            for (int s_i = 0; s_i < sockets; s_i++){
                ...
                send_buffer[i][s_i]->resize(size_of_msg(feature_size) * (partition_offset[i + 1] - partition_offset[i]) * sockets);
                send_buffer[i][s_i]->count = 0;
                recv_buffer[i][s_i]->count = 0;
            }
        } 
...
//        printf("DEBUG NTSCOMM%d\n",i);
    ...
    void emit_buffer(VertexId vtx, ValueType *buffer, int f_size){
        ...
        memcpy(s_buffer + local_send_buffer[t_i]->count * size_of_msg(f_size) + sizeof(VertexId), buffer, sizeof(float) * f_size);
...
//          }
    ...
    void run_all_mirror_to_master(){
        Send=new std::thread([&](){
            send_mirror_to_master();
        });
        Recv=new std::thread([&](){
            recv_mirror_to_master();
        });
    }
...
private:
  ...
  void flush_local_send_buffer_buffer(int t_i,int f_size){
       ...
       // printf("sizeofM<float>(f_size)%d %d %d %d\n",size_of_msg(f_size),local_send_buffer_limit,local_send_buffer[t_i]->count,send_buffer[current_send_part_id][s_i]->count);
        ...
        memcpy(send_buffer[current_send_part_id][s_i]->data + (size_of_msg(f_size)) * pos,
